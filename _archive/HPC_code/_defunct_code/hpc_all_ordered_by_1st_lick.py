...
activity of hippocampus cells ordered by first licks
...
for pathname in pathHPC:
    ...
    trains = np.load('Z:\Dinghao\code_dinghao\HPC_all\{}\HPC_all_info_{}.npy'.format(recname, recname),
                    allow_pickle=True).item()
    
    # load rasters for this recording
    rasters = np.load('Z:\Dinghao\code_dinghao\HPC_all\HPC_all_rasters_npy_simp\{}.npy'.format(recname),
                     allow_pickle=True).item()
    
    # get clunames 
    clulist = list(rasters.keys())
    
    filename = 'Z:/Dinghao/MiceExp/ANMD{}/{}/{}/{}_DataStructure_mazeSection1_TrialType1_alignRun_msess1.mat'.format(recname[1:5], recname[:14], recname, recname)
    ...
    for cluname in clulist:
        ...
        outdirroot_noStim = 'Z:\Dinghao\code_dinghao\HPC_all\{}\single_cell_raster_by_first_licks_noStim'.format(recname)
        if not os.path.exists(outdirroot_noStim):
            os.makedirs(outdirroot_noStim)
        outdirroot = 'Z:\Dinghao\code_dinghao\HPC_all\{}\single_cell_raster_by_first_licks'.format(recname)
        if not os.path.exists(outdirroot):
            os.makedirs(outdirroot)
        
        if noStim=='Y' or noStim=='y':
            fig.savefig('Z:\Dinghao\code_dinghao\HPC_all\{}\single_cell_raster_by_first_licks_noStim\{}.png'.format(recname, cluname),
                        dpi=500,
                        bbox_inches='tight')
        else:
            fig.savefig('Z:\Dinghao\code_dinghao\HPC_all\{}\single_cell_raster_by_first_licks\{}.png'.format(recname, cluname),
                        dpi=500,
                        bbox_inches='tight')
