...
for pathname in pathHPC[18:]:
    ...
    tot_time = 2500  # 2 s after run onsets
    ...
    for i in range(tot_clu):
        if pyr_id[i]==True:
            ...
            for ind, trial in enumerate(cont_trials):
                trial_length = len(rasters[i][trial])-3750
                if trial_length<tot_time and trial_length>0:
                    temp_cont[ind, :trial_length] = rasters[i][trial][3750:6250]
                elif trial_length>0:
                    temp_cont[ind, :] = rasters[i][trial][3750:6250]
            for ind, trial in enumerate(stim_trials):
                trial_length = len(rasters[i][trial])-3750
                if trial_length<tot_time and trial_length>0:
                    temp_stim[ind, :trial_length] = rasters[i][trial][3750:6250]
                elif trial_length>0:
                    temp_stim[ind, :] = rasters[i][trial][3750:6250]
            ...
            for i in range(len(cont_trials)):
                axs[0].scatter(np.where(temp_cont[i]==1)[0]/1250, 
                               [i+1]*int(sum(temp_cont[i])),
                               c='grey', ec='none', s=2)

            for i in range(len(stim_trials)):
                axs[1].scatter(np.where(temp_stim[i]==1)[0]/1250, 
            ...
            outdir = '{}\stim_stimcont_pyr_only_rasters_blowup'.format(outdirroot)
