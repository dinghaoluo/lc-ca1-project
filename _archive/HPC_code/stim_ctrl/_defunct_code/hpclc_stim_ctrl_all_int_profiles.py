...
plot profiles for all interneurones in the HPC-LC stimulation sessions, 
...
#%% parameters
# method for sorting cells; 1: use argmax(), 2: use pre-post firing rate ratio
argmax = 0
...
#%% switch 
if argmax:
    print('using argmax() to sort interneurones...')
    ...
    for pathname in pathHPC:
        ...
        for i, if_int in enumerate(intern_id):
            if if_int:
                # take average 
...
#%% switch 2
    ...
    print('using pre-post spike rate ratios to sort interneurones...')
    ...
    for i in range(tot_clu):
        stim_cont_pp_ratio[i] = np.nanmean(stim_cont[i,625:1875])/np.nanmean(stim_cont[i,3125:4375])  # .5~1.5s
    ...
    for i in range(tot_clu):
        stim_pp_ratio[i] = np.nanmean(stim[i,625:1875])/np.nanmean(stim[i,3125:4375])
    ...
    #%% plotting (argmax)
    fig, ax = plt.subplots(figsize=(5,4))
    ax.set(title='stim_cont_all_int_pre_post_ratio', xlabel='time (s)', ylabel='int #')
    ...
    if HPC_LC:
        fig.suptitle('HPC_LC')
        fig.savefig('Z:\Dinghao\code_dinghao\HPC_all\HPC_LC_stim_all_int_pre_post_ratio.png',
                    dpi=500, bbox_inches='tight')
        fig.savefig('Z:\Dinghao\code_dinghao\HPC_all\HPC_LC_stim_all_int_pre_post_ratio.pdf',
                    bbox_inches='tight')
    elif not HPC_LC:
        fig.suptitle('HPC_LCterm')
        fig.savefig('Z:\Dinghao\code_dinghao\HPC_all\HPC_LCterm_stim_all_int_pre_post_ratio.png',
                    dpi=500, bbox_inches='tight')
        fig.savefig('Z:\Dinghao\code_dinghao\HPC_all\HPC_LCterm_stim_all_int_pre_post_ratio.pdf',
                    bbox_inches='tight')
        
    
    fig, ax = plt.subplots(figsize=(5,4))
    ax.set(title='stim_all_int_pre_post_ratio', xlabel='time (s)', ylabel='int #')
    ...
    if HPC_LC:
        fig.suptitle('HPC_LC')
        fig.savefig('Z:\Dinghao\code_dinghao\HPC_all\HPC_LC_stim_cont_all_int_pre_post_ratio.png',
                    dpi=500, bbox_inches='tight')
        fig.savefig('Z:\Dinghao\code_dinghao\HPC_all\HPC_LC_stim_cont_all_int_pre_post_ratio.pdf',
                    bbox_inches='tight')
    elif not HPC_LC:
        fig.suptitle('HPC_LCterm')
        fig.savefig('Z:\Dinghao\code_dinghao\HPC_all\HPC_LCterm_stim_cont_all_int_pre_post_ratio.png',
                    dpi=500, bbox_inches='tight')
        fig.savefig('Z:\Dinghao\code_dinghao\HPC_all\HPC_LCterm_stim_cont_all_int_pre_post_ratio.pdf',
                    bbox_inches='tight')