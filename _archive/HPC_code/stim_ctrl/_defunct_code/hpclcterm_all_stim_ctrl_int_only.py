...
for pathname in pathHPC:
    ...
    int_id = [label==1 for label in rec_info['isIntern'][0]]  # boolean list for intern labels 
    ...
    tot_clu = len(int_id)
    tot_int = sum(int_id)  # how many interneurones are in this recording
    ...
    for i in range(tot_clu):
        if int_id[i]==True:
            ...
            outdir = '{}\stim_stimcont_int_only'.format(outdirroot)
...
#%% save dataframe 
df.to_csv('Z:\Dinghao\code_dinghao\HPC_all\HPC_LCterm_stim_stimcont_diff_profiles_int_only.csv')