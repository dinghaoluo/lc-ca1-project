...
others = clstr['cluster 2']+clstr['cluster 3']
avg_ro = []; avg_rb = []
burst_ro = []; burst_rb = []

for clu in list(all_tagged_train.items()):
    cluname = clu[0]; print(cluname)
    ...
    for ind in ind_good_beh:
        ...
        # for examples
        if cluname=='A049r-20230121-03 clu9' and ind==13:
            ro_egbef = speed_f['trialsRun'][0][0]['speed_MMsecBef'][0][ind+1][2500:]
            ro_egaft = speed_f['trialsRun'][0][0]['speed_MMsec'][0][ind+1][:5000]
            ro_eg = list(ro_egbef.reshape(-1))+list(ro_egaft.reshape(-1))
    ...
    for i in range(len(run_bout_starts)):
        ...
        if rbsaft>=ro_speed_aft_range[0] and rbsaft<=ro_speed_aft_range[1]:
            ...
            if cluname=='A049r-20230121-03 clu9' and i==17:
                rb_egbef = speed_all[start-1250:start].reshape(-1)
                rb_egaft = speed_all[start:start+5000].reshape(-1)
                rb_eg = list(rb_egbef)+list(rb_egaft)
...
fig.savefig('Z:\Dinghao\code_dinghao\LC_all_tagged\LC_tagged_rovrb_clstr1.png',
            dpi=300,
            bbox_inches='tight')
...
ax.set(xlim=(-1,4),
       ylim=(1.5,6),
       title='trial run onset v run-bout onset (tagged cluster 1)',
       xlabel='time (s)',
       ylabel='spike rate (Hz)')
...
fig.savefig('Z:\Dinghao\code_dinghao\LC_all_tagged\LC_tagged_rovrb_avg_clstr1.png',
            dpi=300,
            bbox_inches='tight')
...
ax.scatter(mean_bro, mean_brb, s=15, color='royalblue', alpha=.9)
...
plt.show()
fig.savefig('Z:\Dinghao\code_dinghao\LC_all_tagged\LC_tagged_rovrb_avg_clstr1_bivariate.png',
            dpi=300,
            bbox_inches='tight')
...
fig.savefig('Z:\Dinghao\code_dinghao\LC_all_tagged\LC_tagged_rovrb_avg_clstr1_bar.png',
            dpi=300,
            bbox_inches='tight')
