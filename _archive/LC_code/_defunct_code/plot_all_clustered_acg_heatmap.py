...
image = ax.imshow(acg_mat, aspect='auto', extent=[-200,200,len(tagged_keys)+1,1], interpolation='none')

ax.set(xlabel='lag (ms)', ylabel='cell #')
plt.colorbar(image, shrink=.5)

fig.suptitle('ACGs tag. $\it{Dbh}$+ cells')
...
fig.savefig('Z:\Dinghao\code_dinghao\LC_all_tagged\LC_all_tagged_ACG_heatmap.png',
         dpi=500,
         bbox_inches='tight')
fig.savefig('Z:\Dinghao\code_dinghao\LC_all_tagged\LC_all_tagged_ACG_heatmap.pdf',
         bbox_inches='tight')
...
image = ax.imshow(acg_mat_put, aspect='auto', extent=[-200,200,len(putative_keys)+1,1], interpolation='none')

ax.set(xlabel='lag (ms)', ylabel='cell #')
plt.colorbar(image, shrink=.5)

fig.suptitle('ACGs put. $\it{Dbh}$+ cells')
...
fig.savefig('Z:\Dinghao\code_dinghao\LC_all\LC_all_putative_Dbh_ACG_heatmap.png',
            dpi=500,
            bbox_inches='tight')
fig.savefig('Z:\Dinghao\code_dinghao\LC_all\LC_all_putative_Dbh_ACG_heatmap.pdf',
            bbox_inches='tight')
...
image = ax.imshow(acg_mat_non, aspect='auto', extent=[-200,200,len(non_keys)+1,1], interpolation='none')
...
fig.savefig('Z:\Dinghao\code_dinghao\LC_all\LC_all_putative_Dbhneg_ACG_heatmap.png',
            dpi=500,
            bbox_inches='tight')
fig.savefig('Z:\Dinghao\code_dinghao\LC_all\LC_all_putative_Dbhneg_ACG_heatmap.pdf',
            bbox_inches='tight')
