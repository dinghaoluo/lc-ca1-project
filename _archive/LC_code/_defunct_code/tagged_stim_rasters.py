...
for pathname in pathLC:
    ...
    if stim_trial.size!=0:
        ...
        i = 0
        for clu in tag_sess:
            clu_name = pathname[-17:]+' clu'+clu+' '+str(stimtype)+' '+str(stimwind[0])+' '+str(stimwind[1])
...
print('\nplotting rasters of all tagged cells in opto-stim session...')
...
fig = plt.figure(1, figsize=[5*4, row_plots*4]); fig.tight_layout()
...
fig.savefig(out_directory + '\\'+'LC_all_tagged_lastStim.png')