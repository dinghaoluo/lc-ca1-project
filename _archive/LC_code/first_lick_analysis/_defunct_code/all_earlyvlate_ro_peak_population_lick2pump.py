...
Modified on Thur 5 Sept 16:06 2024:
    changed to lick-to-pump
    ...
    for cluname in tag_rop_list:
        if cluname[:17]==sessname:
            ...
            for trial in range(tot_trial):
                if len(licks[trial])==0 or len(pumps[trial])==0:
                    continue
                else:
                    pp = pumps[trial][0]
                ...
                if len(lk)==0:
                    lick2pump.append(10000)
                else:
                    lick2pump.append(pp-lk[0])

            temp = list(np.arange(tot_trial))
            l2p_ordered, temp_ordered = zip(*sorted(zip(lick2pump, temp)))
...
fig.savefig(r'Z:\Dinghao\code_dinghao\LC_all\LC_pooled_ROpeak_population_earlyvlate_lick2pump.png',
            bbox_inches='tight',
            dpi=500)
fig.savefig(r'Z:\Dinghao\code_dinghao\LC_all\LC_pooled_ROpeak_population_earlyvlate_lick2pump.pdf',
            bbox_inches='tight')
