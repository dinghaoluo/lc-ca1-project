...
Modified on Mon 2 Sept 16:35 2024:
    changed to first 3 licks and take the median 
    ...
    for cluname in tag_rop_list:
        if cluname[:17]==sessname:
            ...
            for trial in range(tot_trial):
                ...
                if len(lk)<3:
                    first3lick_meds.append(10000)
                else:
                    first3lick_meds.append(np.median([lk[0]-starts[trial], lk[1]-starts[trial], lk[2]-starts[trial]]))

            temp = list(np.arange(tot_trial))
            licks_ordered, temp_ordered = zip(*sorted(zip(first3lick_meds, temp)))
...
fig.savefig(r'Z:\Dinghao\code_dinghao\LC_all\LC_pooled_ROpeak_population_earlyvlate_med3licks.png',
            bbox_inches='tight',
            dpi=500)
fig.savefig(r'Z:\Dinghao\code_dinghao\LC_all\LC_pooled_ROpeak_population_earlyvlate_med3licks.pdf',
            bbox_inches='tight')
