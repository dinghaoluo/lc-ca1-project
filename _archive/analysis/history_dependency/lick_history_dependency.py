...
BEHAVIOUR_STEMS = [
    pp.behaviour_experiment_stem('HPCLC'),
    pp.behaviour_experiment_stem('HPCLCterm'),
    pp.behaviour_experiment_stem('LC'),
    pp.behaviour_experiment_stem('HPCRaphi'),
]
OUTPUT_STEM = pp.FIGURES_ROOT / 'history_dependency' / 'single_session'


#%% helper functions
def save_single_session_figure(fig, output_name, dpi=300):
...
def load_behaviour_session(recname):
    beh_try_paths = [stem / f'{recname}.pkl' for stem in BEHAVIOUR_STEMS]
...
def main():
    ...
    for path in paths:
        ...
        beh = load_behaviour_session(recname)
        if beh is None:
        ...
        ax.set(
            xlabel='deviation from mean (n) (s)',
            ylabel='deviation from mean (n+1) (s)',
        ...
        save_single_session_figure(
            fig,
            f'{recname}_deviation_from_mean.png'
        )
