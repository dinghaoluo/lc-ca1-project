...
for pathname in pathHPC[:-1]:
    ...
    # output routes
    outdirroot = pp.HPC_ALL_FIGURES_STEM / recname
    outdirroot.mkdir(parents=True, exist_ok=True)
    outdir = outdirroot / 'bayesian_decoding'
...
fig.savefig(str(pp.HPC_ALL_FIGURES_STEM / 'bayesian_decoding' / 'stim_stimcont_decoding_mean_error'),
            dpi=500,
            bbox_inches='tight')
...
fig.savefig(
    pp.HPC_ALL_FIGURES_STEM / 'bayesian_decoding' / 'stim_stimcont_decoding_mean_error_vp',
    dpi=500,
    bbox_inches='tight'
)
