...
from common_functions import normalise, normalise_to_all
import project_paths as pp

HPC_ALL_STEM = pp.HPC_ALL_STEM
HPC_ALL_FIGURES_STEM = pp.HPC_ALL_FIGURES_STEM
DIM_REDUCTION_STEM = pp.HPC_ALL_FIGURES_STEM / 'dimensionality_reduction'
...
for pathname in pathHPC:
    ...
    plt.colorbar(ss, shrink=.2, ticks=[], pad=.2)
    # plt.colorbar(sa, shrink=.2, ticks=[], pad=.2)

    fig.tight_layout()
    ax.set(title=recname)
    save_figure(fig, outdirroot / 'stim_stimcont_pca.png',
    ...
    save_figure(fig, outdirroot / 'stim_stimcont_pca_dist.png',
                dpi=500, bbox_inches='tight')
...
avg_dist_ca = np.mean(dist_ca_all, axis=0)
sem_dist_ca = sem(dist_ca_all, axis=0)
avg_dist_sa = np.mean(dist_sa_all, axis=0)
sem_dist_sa = sem(dist_sa_all, axis=0)
...
save_figure(fig, DIM_REDUCTION_STEM / 'pca_dist_stim_stimcont.png',
            dpi=500, bbox_inches='tight')
