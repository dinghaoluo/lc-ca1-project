...
#%% parameters
bins = 18  # bins per trial
xaxis = np.arange(bins)*10


#%% loading
pathname = pp.MICEEXP_ROOT / 'ANMD063r' / 'A063r-20230708' / 'A063r-20230708-01'
...
for clu in range(tot_clu):
    for t in range(tot_trial):
        X_all[clu, t*bins:(t+1)*bins] = skipping_average(alignedRun[clu][t,:1800], skip_step=100)
...
#%% plot
d = list(np.arange(18))*tot_trial  # dist for color mapping

fig = plt.figure()
ax = Axes3D(fig)
plot = ax.scatter(xs=X_emb[:, 0], ys=X_emb[:, 1], zs=X_emb[:, 2], s=3, c=d, cmap=cm.jet)
# cbar = fig.colorbar(plot)

# ax.set(xlim=(29, 34), ylim=(5,10), zlim=(-12,-9))
...
outdir = outdirroot / 'stim_stim_cont_pyr_only_PCA'
...
fig.savefig(f'{outdir}_hist_dist',
            dpi=500,
            bbox_inches='tight')
