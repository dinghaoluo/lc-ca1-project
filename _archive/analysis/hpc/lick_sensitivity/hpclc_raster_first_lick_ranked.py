...
all_info = np.load(pp.HPC_ALL_STEM / 'HPC_all_info.npy',
...
for cluname in clu_list:
    ...
    sessname = cluname[:17]
    rasters = np.load(pp.HPC_ALL_STEM / 'HPC_all_rasters_npy_simp' / f'{sessname}.npy',
                      allow_pickle=True).item()

    raster = rasters[cluname]
    train = all_info[cluname]

    session_stem = pp.MICEEXP_ROOT / f'ANMD{cluname[1:5]}' / cluname[:14] / cluname[:17]
    filename = session_stem / f'{cluname[:17]}_DataStructure_mazeSection1_TrialType1_alignRun_msess1.mat'
    ...
    behParf = session_stem / f'{cluname[:17]}_DataStructure_mazeSection1_TrialType1_behPar_msess1.mat'
    ...
    axs['A'].legend(handles=[fl, rew_line])
    ...
    if noStim=='Y' or noStim=='y':
        outdir = pp.HPC_ALL_FIGURES_STEM / 'single_cell_raster_by_first_licks_noStim'
    else:
        outdir = pp.HPC_ALL_FIGURES_STEM / 'single_cell_raster_by_first_licks'
    outdir.mkdir(parents=True, exist_ok=True)
    fig.savefig(outdir / f'{cluname}.png',
                dpi=300,
                bbox_inches='tight')
