...
for pathname in pathHPC:
    ...
    alignedRun = spike['filteredSpikeDistNormTArrayRun']  # load alignedRun spike data

    profile_cont_dist = np.zeros((tot_pc, 2001))
    profile_stim_dist = np.zeros((tot_pc, 2001))
    for i, cell in enumerate(place_cells):
        # take average
        temp_cont = np.zeros((len(cont_trials), 2001))
        temp_stim = np.zeros((len(stim_trials), 2001))
        for ind, trial in enumerate(cont_trials):
            temp_cont[ind, :] = alignedRun[trial][cell-2]
        for ind, trial in enumerate(stim_trials):
            temp_stim[ind, :] = alignedRun[trial][cell-2]
    ...
    im_mat_cont = np.zeros((tot_pc, 2001))
    im_mat_stim = np.zeros((tot_pc, 2001))
    ...
    # # save figure
    # outdirroot = pp.HPC_ALL_STEM / recname
