...
def get_profiles_and_spike_rates(trains, trials, RO_WINDOW,
                                 ...
                                 SAMP_FREQ=SAMP_FREQ,
                                 BEF=BEF, AFT=AFT):
    return flaf.extract_run_onset_profiles(trains, trials, RUN_ONSET_BIN, SAMP_FREQ, BEF, AFT, rate_window=RO_WINDOW, smoothing_sigma=3, skip_invalid=True)
...
axon_prop = pd.read_pickle(
    pp.LCHPC_AXON_STEM / 'LCHPC_axon_GCaMP_all_profiles.pkl'
    )
...
for path in paths:
    ...
    # load beh
    beh_path = pp.behaviour_experiment_stem('LCHPCGCaMP') / f'{recname}.pkl'
    ...
    # all dFF
    all_dFF = np.load(
        pp.LCHPC_AXON_STEM / 'all_sessions' / recname / 'processed_data' / 'RO_aligned_dict.npy',
        allow_pickle=True
    ).item()
    ...
    for roi in curr_axon_keys:
        ...
        fig.savefig(
            str(pp.LCHPC_AXON_FIGURES_STEM / 'first_lick_analysis' / 'single_axon_early_v_late' / f'{recname} {roi}'),
            dpi=300,
            bbox_inches='tight'
            )
...
if len(sess_early_speed_means_raw) and len(sess_late_speed_means_raw):
    ...
    for ext in ['.png', '.pdf']:
        fig.savefig(
            str(pp.LCHPC_AXON_FIGURES_STEM / 'first_lick_analysis' / f'pre_matching_speed{ext}'),
            dpi=300,
            bbox_inches='tight'
            )
...
if len(sess_early_speed_means) and len(sess_late_speed_means):
    ...
    for ext in ['.png', '.pdf']:
        fig.savefig(
            str(pp.LCHPC_AXON_FIGURES_STEM / 'first_lick_analysis' / f'post_matching_speed{ext}'),
            dpi=300,
            bbox_inches='tight'
            )
...
if early_mean.size and late_mean.size:
    ...
    for ext in ['.png', '.pdf']:
        fig.savefig(
            str(pp.LCHPC_AXON_FIGURES_STEM / 'first_lick_analysis' / f'all_run_onset_mean_profiles_early_v_late{ext}'),
            dpi=300,
            bbox_inches='tight'
            )
