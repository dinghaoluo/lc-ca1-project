...
single_cell_off_stem = pp.LC_EPHYS_FIGURES_STEM / 'single_cell_OFF_v_first_licks_shufbased'
single_cell_on_stem  = pp.LC_EPHYS_FIGURES_STEM / 'single_cell_ON_v_first_licks_shufbased'
single_cell_off_stem.mkdir(parents=True, exist_ok=True)
single_cell_on_stem.mkdir(parents=True, exist_ok=True)
(pp.LC_OPTO_EPHYS_STEM).mkdir(parents=True, exist_ok=True)
...
#%% load dataframe
cell_prop = pd.read_pickle(pp.LC_EPHYS_STEM / 'LC_all_single_cell_properties.pkl')


#%% load trains
all_train = np.load(pp.LC_EPHYS_STEM / 'LC_all_info.npy',
...
for cluname in OFF:
    ...
    # get stim trials
    behInfo = sio.loadmat(rec_stem / f'{recname}_DataStructure_mazeSection1_TrialType1_Info.mat')['beh']
    ...
    for trial in range(tot_trial):
        ...
        if tlick<10000:  # only execute if there is actually licks in this trial
            ...
            try:
                ...
                outdir = str(pp.LC_EPHYS_FIGURES_STEM / 'lick_sensitivity' / f"lick_sensitive_cutoff_point_analysis_shufbased{cluname + ' OFF'}trial{trial + 1}.png")
    ...
    fig.savefig(single_cell_off_stem / f"{cluname + ' OFF'}.png",
                dpi=500,
                bbox_inches='tight')
...
off_summary_stem = pp.LC_OPTO_EPHYS_FIGURES_STEM / 'opto_020_time_to_OFF_shufbased'
off_summary_stem.parent.mkdir(parents=True, exist_ok=True)
pf.plot_violin_with_scatter(mean_cutoff_all_clean, mean_cutoff_stim_clean,
                            ...
                            ylabel='t. to OFF',
                            title='1st-lick OFF',
                            save=True, savepath=off_summary_stem, dpi=300)
...
for cluname in ON:
    ...
    for trial in range(tot_trial):
        ...
        if tlick<10000:  # only execute if there is actually licks in this trial
            ...
            try:
                ...
                ax.axvspan((co-tlick-1)/1250, (co-tlick+1)/1250, color='r')
                ax.set(title='trial {}'.format(trial+1))
                outdir = str(pp.LC_EPHYS_FIGURES_STEM / 'lick_sensitivity' / f"lick_sensitive_cutoff_point_analysis_shufbased{cluname + ' ON'}trial{trial + 1}.png")
                os.makedirs(os.path.dirname(outdir), exist_ok=True)
                fig.savefig(outdir,
                            dpi=100, bbox_inches='tight')
                plt.close(fig)
    ...
    fig.savefig(single_cell_on_stem / f"{cluname + ' ON'}.png",
                dpi=500,
                bbox_inches='tight')
...
on_summary_stem = pp.LC_OPTO_EPHYS_FIGURES_STEM / 'opto_020_time_to_ON_shufbased'
on_summary_stem.parent.mkdir(parents=True, exist_ok=True)
pf.plot_violin_with_scatter(mean_cutoff_all_clean, mean_cutoff_stim_clean,
                            'grey', 'darkorange',
                            paired=True,
                            xticklabels=['ctrl.', 'stim'],
                            ylabel='t. to ON',
                            title='1st-lick ON',
                            save=True, savepath=on_summary_stem, dpi=300)
