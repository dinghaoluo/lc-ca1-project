...
all_train = np.load(pp.LC_ALL_STEM / 'LC_all_info.npy',
...
#%% main
warped_dict = {}
warped_output_stem = pp.LC_ALL_FIGURES_STEM / 'single_cell_warped_RO_first_licks'
warped_output_stem.mkdir(parents=True, exist_ok=True)
...
for cluname in clu_list:
    ...
    for ext in ['png', 'pdf']:
        fig.savefig(warped_output_stem / '{}.{}'.format(title, ext),
                    bbox_inches='tight',
                    dpi=200)
...
#%% save
np.save(pp.LC_ALL_STEM / 'LC_all_warped.npy',
        warped_dict)
