...
beh_df = pd.read_pickle(pp.behaviour_session_pickle('all_LC_sessions.pkl'))
df = pd.read_pickle(pp.LC_EPHYS_STEM / 'LC_all_cell_profiles.pkl')
...
burst_prob_early = []
burst_prob_late = []
burst_probability_stem = pp.LC_EPHYS_FIGURES_STEM / 'first_lick_analysis' / 'burst_probability'
burst_probability_stem.parent.mkdir(parents=True, exist_ok=True)
...
for path in paths:
    ...
    burst_dict = np.load(
        sess_folder / f'{recname}_all_bursts.npy',
        allow_pickle=True
        ).item()
    spike_dict = np.load(
        sess_folder / f'{recname}_all_spikes.npy',
        allow_pickle=True
        ).item()
...
plot_violin_with_scatter(burst_prob_early, burst_prob_late,
                         ...
                         showscatter=False,
                         save=True,
                         savepath=burst_probability_stem)
...
for ext in ('.png', '.pdf'):
    fig.savefig(
        burst_probability_stem.parent / f'burst_probability_box{ext}',
        dpi=300,
        bbox_inches='tight'
        )
