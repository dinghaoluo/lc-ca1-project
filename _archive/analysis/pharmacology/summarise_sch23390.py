...
from lick_time_utils import mean_lick_time_profile, plot_lick_time_profile
...
pharmacology_stem = pp.PHARMACOLOGY_STEM
...
for pathname, sesslist in zip(pathSCH, sessSCH):
    ...
    for sess in sesslist:
        ...
        mean_lick_time = mean_lick_time_profile(file['lick_times_aligned'])

        if sess == 1:
            lick_baseline = mean_licks
            mean_licks_baseline.append(mean_licks)
            mean_lick_times_baseline.append(mean_lick_time)
        elif sess in [2, 3]:
            lick_drugs.append(mean_licks)
            lick_time_drugs.append(mean_lick_time)
...
#%% LICK TIME PLOT
plot_lick_time_profile(
    mean_lick_times_baseline,
    mean_lick_times_drug,
    '#004D80',
    'SCH23390',
    pharmacology_stem / 'SCH23390',
    filename='lick_time_profile'
)
