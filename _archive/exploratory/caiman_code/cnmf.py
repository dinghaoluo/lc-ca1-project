...
def run():
    ...
    p_ops = Path(r'Z:\Jingyu\2P_Recording\AC926\AC926-20240305\02\RegOnly\suite2p\plane0\ops.npy')
    try:
        p_memmap = next(p_ops.parent.glob('memmap_*'))
        print(f'Found memmap file. Using: {p_memmap}')
    except StopIteration:
        p_memmap = utl.save_data_as_mmap(p_ops, crop=True)
    ...
    method_init = 'greedy_roi'  # initialization method (for dendritic data, see CaImAn dendritic examples)
