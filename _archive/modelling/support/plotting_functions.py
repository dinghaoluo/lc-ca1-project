...
from common_functions import show_plot
...
def plot_box_with_scatter(ctrl_data, stim_data, xlabel, savepath, 
    ...
    if show_scatter:
        ax.scatter(stim_data, np.full_like(stim_data, 1.7),
                   s=20, color=stim_color, alpha=0.75, ec='none')
        ax.scatter(ctrl_data, np.full_like(ctrl_data, 1.3),
                   s=20, color=ctrl_color, alpha=0.75, ec='none')
...
def plot_violin_with_scatter(data0, data1, colour0, colour1,
    ...
    '''
    stats_labels = xticklabels if xticklabels is not None else ['data0', 'data1']
    label0 = str(stats_labels[0]) if len(stats_labels) > 0 else 'data0'
    label1 = str(stats_labels[1]) if len(stats_labels) > 1 else 'data1'
    ...
    if showmeans:
        ...
        ax.scatter(1.25, np.mean(data0), 
                   s=30, color=colour0, ec='none', lw=.5, zorder=3)
        ax.scatter(1.75, np.mean(data1), 
                   s=30, color=colour1, ec='none', lw=.5, zorder=3)
    ...
    if showmedians:
        ...
        ax.scatter(1.25, np.median(data0), 
                   s=30, color=colour0, ec='none', lw=.5, zorder=2)
        ax.scatter(1.75, np.median(data1), 
                   s=30, color=colour1, ec='none', lw=.5, zorder=2)
        if paired and showline:
            ax.plot([1.25, 1.75], 
                    [data0, data1], 
                    color='grey', alpha=alpha, linewidth=1, zorder=1)
...
def plot_ecdfs(data0, data1, 
               ...
               legend_labels=[' ', ' '],
               colours=[(240 / 255, 128 / 255, 128 / 255),
                        (178 / 255, 34 / 255, 34 / 255)],
...
def get_lower_upper_bounds_violin(body):
    '''take a violinplot body and return its lower and upper bounds'''
