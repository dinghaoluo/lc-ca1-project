...
decay-time extraction for PyrUp and PyrDown profiles following Heldman et al.
(2025)
...
#%% functions
def _save_figure_if_requested(fig, filename):
    if filename is None:
        return
...
def compute_tau(time: np.array,
    ...
    - cell_type: string indicating cell response type; default is PyrUp.

    returns:
    - tau: estimated decay time constant (in seconds), or None if fitting fails
    - fit_params: dictionary containing fit parameters ('a', 'b'), r_squared, adj_r_squared, and baseline; or None if fitting fails
    '''
    time = np.array(time)
...
def detect_min_max(
        ...
        run_onset_bin=1250,
        SAMP_FREQ=1250):
    if cell_type == PYRUP_CLASS:
        return np.argmax(profile[run_onset_bin:run_onset_bin+SAMP_FREQ*3])
    elif cell_type == PYRDOWN_CLASS:
        return np.argmin(profile[run_onset_bin:run_onset_bin+SAMP_FREQ*3])
    else:
        raise ValueError('cell_type must be PyrUp or PyrDown')

def detect_peak(profile, cell_type=PYRUP_CLASS,
    ...
    - cell_type: str, PyrUp or PyrDown archive class value
...
def plot_fit(time: np.array,
             ...
             filename=None):
    '''
    plot one fitted decay curve and optionally save it when `filename` is provided.
    '''
    x_data = time[peak_idx:] - time[peak_idx]
    y_data = profile[peak_idx:]
    ...
    _save_figure_if_requested(fig, filename)
...
def plot_fit_compare(time: np.array,
                     ...
                     SAVE=True):
    '''
    plot two fitted decay curves and optionally save them when `filename` is provided.
    '''
...
def main():
    ...
    output_dir = pp.FIGURES_ROOT / 'hpc_ephys' / 'single_cell_decay_time'
    ...
    for cluname, clu in df_classified.iterrows():
        ...
        # get peak or trough
        peak_idx = detect_min_max(mean_prof, clu_class)
        ...
        plot_fit(
            ...
            cluname,
            clu_class,
            filename=output_dir / f'{cluname} {clu_class}.png'
            )
