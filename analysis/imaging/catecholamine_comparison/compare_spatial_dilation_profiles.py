...
compare dopamine and norepinephrine spatial spread around releasing axon ROIs
...
from scipy.stats import ranksums, sem, ttest_1samp
import matplotlib
matplotlib.use('Agg')
...
for dataset in DATASETS:
    ...
    for path in dataset['paths']:
        ...
        for roi_id, roi in roi_dict.items():
            means = np.nanmean(pixel_RI_stim[roi['ypix'], roi['xpix'], :], axis=0)
            means = means[np.isfinite(means)]
            if means.size < MIN_VALID_TRIALS:
                continue
            _, p_val = ttest_1samp(means, 0, alternative='greater')
            if p_val < ALPHA and np.mean(means) > MIN_RI:
        ...
        if n_raw == 0 or n_thres == 0:
            print_status('skipped', 'no releasing ROI pixels after mask')
            continue
        ...
        for dilation in range(0, MAX_DILATE + 1, DILATE_STEP):
            ...
            for wname, bins in WINDOW_BINS.items():
                ...
                session_curve_rows.append({
                    ...
                    'n_roi_releasing': len(releasing_rois),
        ...
        print_status('done', f'{len(releasing_rois)} releasing ROIs')
        curve_rows.extend(session_curve_rows)
        tau_rows.extend(session_tau_rows)


#%% save tables
curve_csv = data_stem / 'spatial_dilation_curves.csv'
tau_csv = data_stem / 'spatial_tau_metrics.csv'
...
for ext in ['.png', '.pdf']:
    out = save_stem / f'dopamine_norepinephrine_spatial_dilation_curves{ext}'
...
for ext in ['.png', '.pdf']:
    out = save_stem / f'dopamine_norepinephrine_spatial_dilation_curves_normalised{ext}'
    fig.savefig(out, dpi=300, bbox_inches='tight')
    saved.append((ext[1:], out))
plt.close(fig)
...
fig.tight_layout(w_pad=0.7)
for ext in ['.png', '.pdf']:
    out = save_stem / f'dopamine_norepinephrine_spatial_tau{ext}'
    fig.savefig(out, dpi=300, bbox_inches='tight')
    saved.append((ext[1:], out))
plt.close(fig)
