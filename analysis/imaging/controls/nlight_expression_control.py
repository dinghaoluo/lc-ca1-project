...
control nLight expression against stimulation response
...
save_stem = pp.HPC_NLIGHT_LC_OPTO_FIGURES_STEM / 'controls'
(save_stem / 'single_session_nLight_expression_corr').mkdir(parents=True, exist_ok=True)
...
for path in paths:
    ...
    ref1 = np.load(proc_data_path / 'ref_mat_ch1.npy', allow_pickle=True)
    roi_dict = np.load(roi_dict_path, allow_pickle=True).item()
    pixel_RI = np.load(pixel_RI_stim_path, allow_pickle=True)
    ...
    r = np.corrcoef(ref1.ravel(), pixel_RI_med.ravel())[0, 1]
    all_r.append(r)

    # ROI-specific correlation
    r_axon = np.corrcoef(ref1[all_roi_mask], pixel_RI_med[all_roi_mask])[0, 1]
    all_r_axon.append(r_axon)

    # regression (for plotting)
    m, b, *_ = linregress(ref1.ravel(), pixel_RI_med.ravel())

    # permutation tests
    x_all = ref1.ravel()
    y_all = pixel_RI_med.ravel()
    y_ax = pixel_RI_med[all_roi_mask]
    ...
    ax.set_xlabel('nLight intensity')
    ...
    for ext in ['.png', '.pdf']:
        fig.savefig(
            save_stem / f'single_session_nLight_expression_corr/{recname}_scatter{ext}',
            dpi=300,
            bbox_inches='tight'
        )
    plt.close(fig)
    ...
    fig, ax = plt.subplots(figsize=(2.4, 2.2))
    ax.scatter(ref1[all_roi_mask], pixel_RI_med[all_roi_mask],
               color='mediumseagreen', edgecolor='none', s=8, alpha=0.7, rasterized=True)

    # regression line for axon pixels
    m_ax, b_ax, *_ = linregress(ref1[all_roi_mask], pixel_RI_med[all_roi_mask])
    ...
    ax.set_xlabel('nLight intensity (axon ROIs)')
    ...
    for ext in ['.png', '.pdf']:
        fig.savefig(
            save_stem / f'single_session_nLight_expression_corr/{recname}_axon_scatter{ext}',
            dpi=300,
            bbox_inches='tight'
        )
    plt.close(fig)
...
print(f'all pixels: median R^2 = {med_r2:.4f} +/- {iqr_r2/2:.4f}')
print(f'axon ROIs: median R^2 = {med_r2_axon:.4f} +/- {iqr_r2_axon/2:.4f}')
...
for ext in ['.png', '.pdf']:
    fig.savefig(save_stem / f'all_sessions_nLight_expression_R2_summary{ext}',
