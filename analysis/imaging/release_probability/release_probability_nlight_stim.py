...
quantify nLight release probability by axonal ROI
...
from common_functions import smooth_convolve, mpl_formatting
...
pp.HPC_NLIGHT_LC_OPTO_FIGURES_STEM.mkdir(parents=True, exist_ok=True)
...
for path in paths:
    ...
    pixel_RI_path  = proc_data_path / f'{recname}_pixel_RI_stim.npy'
    roi_dict_path  = proc_data_path / f'{recname}_ROI_dict.npy'
    ref2_path      = proc_data_path / 'ref_mat_ch2.npy'
    ref1100_path   = proc_data_path / f'{recname}_ref_mat_1100nm.npy'
    ...
    if ref1100_path.exists():
        print_status('using 1100-nm ref.')
        ref2 = np.load(ref1100_path, allow_pickle=True)
    else:
        print_status('using channel 2 ref.')
    ...
    fig.colorbar(im, shrink=0.25, ticks=[global_min, 0, global_max])

    fig.tight_layout()

    session_fig_stem = all_sess_fig_stem / recname
    session_fig_stem.mkdir(parents=True, exist_ok=True)

    out_path = session_fig_stem / f'{recname}_releasing_ROI_map'
    for ext in ['.png', '.pdf']:
        fig.savefig(
            f'{out_path}{ext}',
            dpi=300,
    ...
    # ----------------------------
    print_status('loading F_aligned array')
    ...
    # iterate
    save_stem = session_fig_stem / 'single_ROI_stim_aligned'
    ...
    for roi_id, roi in tqdm(roi_dict.items(),
        ...
        for tr in range(n_trials):
            ...
            for shuf in range(N_SHUF):
                ...
                shuf_trace_arr[shuf, :] = smooth_convolve(
                    shuf_trace, sigma=SAMP_FREQ/10
                )
    ...
    if not sess_release_probabilities:
        print_status('skipped', 'no valid ROI for session')
        continue
    ...
    # save
    summary_path = session_fig_stem / f'{recname}_release_prob_hist'
    for ext in ['.png', '.pdf']:
        fig.savefig(
            f'{summary_path}{ext}',
            dpi=300,
...
for ext in ['.pdf', '.png']:
    fig.savefig(
        pp.HPC_NLIGHT_LC_OPTO_FIGURES_STEM / f'nLight_LC_stim_release_probabilities{ext}',
        dpi=300, bbox_inches='tight')
plt.close(fig)
