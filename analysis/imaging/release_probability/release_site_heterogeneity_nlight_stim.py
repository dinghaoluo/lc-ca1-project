...
analyse nLight release-site heterogeneity
...
nLight_stem   = pp.HPC_NLIGHT_LC_OPTO_STEM
all_sess_stem = nLight_stem / 'all_sessions'
fig_stem      = pp.HPC_NLIGHT_LC_OPTO_RELEASE_SITE_HETEROGENEITY_FIGURES_STEM
...
def plot_real_vs_shuffle(results, key, shuffle_key, ylabel, title, savename):
    curr = [res for res in results if res['condition'] == 'nLight']
...
def plot_heterogeneity_summary(results):
    ...
    fig_stem.mkdir(parents=True, exist_ok=True)
    for ext in ['.png', '.pdf']:
        fig.savefig(fig_stem / f'nLight_release_site_heterogeneity_summary{ext}', dpi=300, bbox_inches='tight')
    plt.close(fig)
...
if SAVE_FIGURES:
    plot_real_vs_shuffle(
        ...
        ylabel='top 10% contribution',
        title='response concentration',
        savename='nLight_top10_contribution_real_vs_shuffle'
        )
    plot_real_vs_shuffle(
        all_results,
        key='split_half_r',
        ...
        ylabel='odd-even Spearman r',
        title='hotspot stability',
        savename='nLight_split_half_stability_real_vs_shuffle'
        )
...
#%% optional save
if SAVE_OUTPUT:
    out_path = pp.HPC_NLIGHT_LC_OPTO_STEM / 'release_site_heterogeneity_nlight_stim.csv'
