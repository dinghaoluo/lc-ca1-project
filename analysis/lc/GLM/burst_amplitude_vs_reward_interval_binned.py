...
Originally named amp_since_last_reward_binned.py
...
from scipy.stats import sem, wilcoxon, linregress, ttest_1samp
...
BASELINE_ON = 0.55
BASELINE_OFF = 0.45
BASELINE_LONG_ON = 0.75
BASELINE_LONG_OFF = 0.25
...
for output_dir in [
    'rew_to_run_baseline_single_session',
    'rew_to_run_baseline_single_session_long',
    'baseline_vs_phasic_single_session',
    'baseline_vs_phasic_single_session_long',
]:
    (GLM_stem / output_dir).mkdir(parents=True, exist_ok=True)
...
for path in paths:
    ...
    # trial-statement protocol field identifies optogenetic trials
    opto_idx = [i for i, t in enumerate(trials_sts) if t[15] != '0']

    # reward- and run-aligned sample indices into the full spike maps
    ...
    for ti in valid_trials:
        ...
        for cluname in eligible_cells:
            ...
            trace_rew = smooth_convolve(trace_rew, sigma=SAMP_FREQ*0.2, axis=0)
            ...
            trace_run = smooth_convolve(trace_run, sigma=SAMP_FREQ*0.2, axis=0)
            all_trials_by_cell_run[cluname].append((t_since, trace_run))

            # 100-ms baseline centred 0.5 s before run onset
            ...
            if t_since > 1:
                # long baseline spans 0.75 to 0.25 s before run onset
                x_rew = np.arange(len(trace_rew)) / SAMP_FREQ - PRE_REW_S
...
ax.scatter(
    np.ones(len(sess_regress_r_baseline_phasic_long)),
    sess_regress_r_baseline_phasic_long,
    color='forestgreen',
    ec='none',
    s=10,
