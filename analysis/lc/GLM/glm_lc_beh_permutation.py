# -*- coding: utf-8 -*-
'''
Created on Mon Oct 13 11:46:03 2025

GLM to test predictors of LC run-onset amplitude
    (baseline & recovery trials, unmanipulated sessions)
    this script uses permutation (instead of Wilcoxon) for signif. testing
...
for path in paths:
    ...
    for cluname, row in curr_cell_prop.iterrows():
        ...
        base_5_rows = []
        for ti in kept_trials:
            base_start = int(run_onsets_spike[ti] - SAMP_FREQ * 5)
            base_end   = int(run_onsets_spike[ti] - SAMP_FREQ * 2.5)
            base_5 = np.nanmean(spike_maps[clu_idx, base_start : base_end])
        ...
        if X_diag.shape[0] >= 3:
            ...
            if Xc.shape[1] >= 2:
                ...
                gpf.save_collinearity_heatmap(
                    corr,
                    cluname,
                    GLM_stem / 'collinearity_heatmaps' / f'{cluname}_corr_heatmap'
                )
...
plt.tight_layout()
for ext in ['.pdf', '.png']:
    fig.savefig(GLM_stem / f'deltaR2_by_predictor_permutation{ext}',
                dpi=300,
                bbox_inches='tight')
...
plt.tight_layout()
for ext in ['.pdf', '.png']:
    fig.savefig(GLM_stem / f'model_AIC_comparison_permutation{ext}',
                dpi=300, bbox_inches='tight')
...
plt.tight_layout()
for ext in ['.pdf', '.png']:
    fig.savefig(GLM_stem / f'likelihood_ratio_stat_permutation{ext}',
                dpi=300, bbox_inches='tight')
