...
Originally named 1st_lick_profile.py
...
#%% distance
output_stem = pp.LC_ALL_FIGURES_STEM
...
fig.savefig(output_stem / 'LC_first_lick_hist_dist.png',
            dpi=500,
            bbox_inches='tight')
fig.savefig(output_stem / 'LC_first_lick_hist_dist.pdf',
            dpi=500,
            bbox_inches='tight')
...
for pathname in pathOpt:
    ...
    for trial in range(tot_trial):
        ...
        if trial+1 in use_trials:
            lk = [l[0] for l in licks[trial] if l-starts[trial] > 1250]  # exclude licks in the 1st second, as they could be carry-over licks from the last trial
            pump_delta = pumps[trial]-starts[trial]
            if len(lk)!=0 and len(pump_delta)!=0:  # append only if there is licks in this trial
                all_first_licks_time.append([lk[0]-starts[trial]])
                all_pumps_time.append(pump_delta[0][0])
...
fig.savefig(output_stem / 'LC_first_lick_hist_time.png',
            dpi=500,
            bbox_inches='tight')
fig.savefig(output_stem / 'LC_first_lick_hist_time.pdf',
            dpi=500,
            bbox_inches='tight')
