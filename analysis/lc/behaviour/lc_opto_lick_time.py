...
Originally named lick_time_comp_020.py
...
for sessname in sess_list:
    ...
    # licks plot
    if comp_method == 'baseline':
        savepath = str(pp.LC_OPTO_EPHYS_FIGURES_STEM / f'opto_licktime_0{stim_cond}0' / sessname)
        Path(savepath).parent.mkdir(parents=True, exist_ok=True)
    elif comp_method == 'stim_cont':
        savepath = str(pp.LC_OPTO_EPHYS_FIGURES_STEM / f'opto_licktime_0{stim_cond}0_stim_cont' / sessname)
        Path(savepath).parent.mkdir(parents=True, exist_ok=True)
    ...
    # mspeeds plot
    if comp_method == 'baseline':
        savepath = str(pp.LC_OPTO_EPHYS_FIGURES_STEM / f'opto_licktime_0{stim_cond}0' / f'{sessname}_control_velocity')
        Path(savepath).parent.mkdir(parents=True, exist_ok=True)
    elif comp_method == 'stim_cont':
        savepath = str(pp.LC_OPTO_EPHYS_FIGURES_STEM / f'opto_licktime_0{stim_cond}0_stim_cont' / f'{sessname}_control_velocity')
        Path(savepath).parent.mkdir(parents=True, exist_ok=True)
    ...
    # pspeeds plot
    if comp_method == 'baseline':
        savepath = str(pp.LC_OPTO_EPHYS_FIGURES_STEM / f'opto_licktime_0{stim_cond}0' / f'{sessname}_control_peak_velocity')
        Path(savepath).parent.mkdir(parents=True, exist_ok=True)
    elif comp_method == 'stim_cont':
        savepath = str(pp.LC_OPTO_EPHYS_FIGURES_STEM / f'opto_licktime_0{stim_cond}0_stim_cont' / f'{sessname}_control_peak_velocity')
        Path(savepath).parent.mkdir(parents=True, exist_ok=True)
    ...
    # init acc plot
    if comp_method == 'baseline':
        savepath = str(pp.LC_OPTO_EPHYS_FIGURES_STEM / f'opto_licktime_0{stim_cond}0' / f'{sessname}_control_init_accel')
        Path(savepath).parent.mkdir(parents=True, exist_ok=True)
    elif comp_method == 'stim_cont':
        savepath = str(pp.LC_OPTO_EPHYS_FIGURES_STEM / f'opto_licktime_0{stim_cond}0_stim_cont' / f'{sessname}_control_init_accel')
        Path(savepath).parent.mkdir(parents=True, exist_ok=True)
    ...
    # lick-bef-rew plot
    if comp_method == 'baseline':
        savepath = str(pp.LC_OPTO_EPHYS_FIGURES_STEM / f'opto_licktime_0{stim_cond}0' / f'{sessname}_control_licks_bef_rew')
        Path(savepath).parent.mkdir(parents=True, exist_ok=True)
    elif comp_method == 'stim_cont':
        savepath = str(pp.LC_OPTO_EPHYS_FIGURES_STEM / f'opto_licktime_0{stim_cond}0_stim_cont' / f'{sessname}_control_licks_bef_rew')
        Path(savepath).parent.mkdir(parents=True, exist_ok=True)
    ...
    # trial-length plot
    if comp_method == 'baseline':
        savepath = str(pp.LC_OPTO_EPHYS_FIGURES_STEM / f'opto_licktime_0{stim_cond}0' / f'{sessname}_control_trial_length')
        Path(savepath).parent.mkdir(parents=True, exist_ok=True)
    elif comp_method == 'stim_cont':
        savepath = str(pp.LC_OPTO_EPHYS_FIGURES_STEM / f'opto_licktime_0{stim_cond}0_stim_cont' / f'{sessname}_control_trial_length')
        Path(savepath).parent.mkdir(parents=True, exist_ok=True)
...
#%% licks summary
if comp_method == 'baseline':
    savepath = str(pp.LC_OPTO_EPHYS_FIGURES_STEM / 'opto_licktime_020' / 'summary')
    Path(savepath).parent.mkdir(parents=True, exist_ok=True)
elif comp_method == 'stim_cont':
    savepath = str(pp.LC_OPTO_EPHYS_FIGURES_STEM / 'opto_licktime_020_stim_cont' / 'summary')
    Path(savepath).parent.mkdir(parents=True, exist_ok=True)
...
#%% mean speeds summary
if comp_method == 'baseline':
    savepath = str(pp.LC_OPTO_EPHYS_FIGURES_STEM / 'opto_licktime_020' / 'summary_control_velocity')
    Path(savepath).parent.mkdir(parents=True, exist_ok=True)
elif comp_method == 'stim_cont':
    savepath = str(pp.LC_OPTO_EPHYS_FIGURES_STEM / 'opto_licktime_020_stim_cont' / 'summary_control_velocity')
    Path(savepath).parent.mkdir(parents=True, exist_ok=True)
...
#%% peak speeds summary
if comp_method == 'baseline':
    savepath = str(pp.LC_OPTO_EPHYS_FIGURES_STEM / 'opto_licktime_020' / 'summary_control_peak_velocity')
    Path(savepath).parent.mkdir(parents=True, exist_ok=True)
elif comp_method == 'stim_cont':
    savepath = str(pp.LC_OPTO_EPHYS_FIGURES_STEM / 'opto_licktime_020_stim_cont' / 'summary_control_peak_velocity')
    Path(savepath).parent.mkdir(parents=True, exist_ok=True)
...
#%% init accels summary
if comp_method == 'baseline':
    savepath = str(pp.LC_OPTO_EPHYS_FIGURES_STEM / 'opto_licktime_020' / 'summary_control_init_accel')
    Path(savepath).parent.mkdir(parents=True, exist_ok=True)
elif comp_method == 'stim_cont':
    savepath = str(pp.LC_OPTO_EPHYS_FIGURES_STEM / 'opto_licktime_020_stim_cont' / 'summary_control_init_accel')
    Path(savepath).parent.mkdir(parents=True, exist_ok=True)
...
if comp_method == 'baseline':
    savepath = str(pp.LC_OPTO_EPHYS_FIGURES_STEM / 'opto_licktime_020' / 'summary_control_licks_bef_rew')
    Path(savepath).parent.mkdir(parents=True, exist_ok=True)
elif comp_method == 'stim_cont':
    savepath = str(pp.LC_OPTO_EPHYS_FIGURES_STEM / 'opto_licktime_020_stim_cont' / 'summary_control_licks_bef_rew')
    Path(savepath).parent.mkdir(parents=True, exist_ok=True)
...
if comp_method == 'baseline':
    savepath = str(pp.LC_OPTO_EPHYS_FIGURES_STEM / 'opto_licktime_020' / 'summary_control_trial_length')
    Path(savepath).parent.mkdir(parents=True, exist_ok=True)
elif comp_method == 'stim_cont':
    savepath = str(pp.LC_OPTO_EPHYS_FIGURES_STEM / 'opto_licktime_020_stim_cont' / 'summary_control_trial_length')
    Path(savepath).parent.mkdir(parents=True, exist_ok=True)
...
if comp_method == 'baseline':
    savepath = str(pp.LC_OPTO_EPHYS_FIGURES_STEM / 'opto_licktime_020' / 'summary_control_rew_perc')
    Path(savepath).parent.mkdir(parents=True, exist_ok=True)
elif comp_method == 'stim_cont':
    savepath = str(pp.LC_OPTO_EPHYS_FIGURES_STEM / 'opto_licktime_020_stim_cont' / 'summary_control_rew_perc')
    Path(savepath).parent.mkdir(parents=True, exist_ok=True)
