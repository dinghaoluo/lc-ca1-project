...
Originally named all_runonset_runbout_ro_peak.py
Modified on Wed 26 Feb 2025 for bug fixes
...
LC_EPHYS_STEM = pp.LC_EPHYS_STEM
RUN_BOUT_STEM = pp.RUN_BOUTS_STEM
RUN_ONSET_RUN_BOUT_STEM = pp.LC_EPHYS_FIGURES_STEM / 'run_onset_v_run_bout'
RUN_ONSET_RUN_BOUT_SINGLE_CELL_STEM = RUN_ONSET_RUN_BOUT_STEM / 'single_cell'
BEHAVIOUR_OUTPUT_STEM = pp.BEHAVIOUR_FIGURES_STEM / 'LC_run_onset_run_bout'
CELL_PROFILES_PATH = LC_EPHYS_STEM / 'LC_all_cell_profiles.pkl'
BEHAVIOUR_PATH = pp.BEHAVIOUR_STEM / 'all_LC_sessions.pkl'
RUN_ONSET_RUN_BOUT_SINGLE_CELL_STEM.mkdir(parents=True, exist_ok=True)
BEHAVIOUR_OUTPUT_STEM.mkdir(parents=True, exist_ok=True)
...
#%% speed correction and identity analysis


def interpolate_negative_speed_values(speed_values):
    speed_values = list(speed_values)
    for tbin in range(len(speed_values)):
        if speed_values[tbin] < 0:
            if tbin == len(speed_values) - 1:
                speed_values[tbin] = speed_values[tbin - 1]
            elif tbin == 0:
                speed_values[tbin] = speed_values[tbin + 1]
            else:
                speed_values[tbin] = (
                    speed_values[tbin - 1] + speed_values[tbin + 1]
                    ) / 2
    return speed_values


#%% analysis
def analyse_identity(keys, list_identity):
    ...
    for cluname in keys:
        ...
        if temp_recname != recname:
            ...
            all_trains = np.load(
                LC_EPHYS_STEM / 'all_sessions' / recname / f'{recname}_all_trains_run.npy',
                allow_pickle=True).item()

            pathname = str(
                MICEEXP_ROOT / f'ANMD{cluname[1:5]}' / recname[:14] / recname
            )

            # get trial types
            trial_statements = behaviour.loc[recname]['trial_statements']
            stim_conds = [trial[15] for trial in trial_statements][1:]  # index 15 is the stim condition
            ...
            for idx in good_idx:
                ...
                curr_bef_speed = interpolate_negative_speed_values(curr_bef_speed)
            ...
            run_bout_file = mat73.loadmat(
                RUN_BOUT_STEM / 'fsa_run_bouts' / f'{recname}_BefRunBout0.mat'
                )
            run_bout_table = pd.read_csv(
                RUN_BOUT_STEM / f'{recname}_run_bouts_py.csv'
                )
            ...
            for i in range(len(run_bout_starts)):
                ...
                curr_aft_runbout_speed = interpolate_negative_speed_values(
                    curr_aft_runbout_speed
                    )
            ...
            if len(matched_bouts) > 0:
                ...
                for ext in ['.png', '.pdf']:
                    fig.savefig(
                        BEHAVIOUR_OUTPUT_STEM / f'{recname}_matched_bouts{ext}',
                        dpi=300,
                        bbox_inches='tight'
                        )
        ...
        fig.savefig(
            RUN_ONSET_RUN_BOUT_SINGLE_CELL_STEM / f'{cluname}.png',
            dpi=300,
            bbox_inches='tight'
            )
    ...
    for ext in ['.png', '.pdf']:
        fig.savefig(
            RUN_ONSET_RUN_BOUT_STEM / f'LC_{list_identity}_Dbh_ROpeaking_run_onset_run_bout{ext}',
            dpi=300,
            bbox_inches='tight'
            )
    ...
    pf.plot_violin_with_scatter(s2n_run_onset, s2n_run_bout,
                                ...
                                title='LC SNR',
                                save=True,
                                savepath=str(RUN_ONSET_RUN_BOUT_STEM / f'LC_{list_identity}_Dbh_ROpeaking_run_onset_run_bout_SNR_violin'),
                                dpi=300)
    ...
    pf.plot_violin_with_scatter(RO_run_onset, RO_run_bout,
                                ...
                                save=True,
                                savepath=str(RUN_ONSET_RUN_BOUT_STEM / f'LC_{list_identity}_Dbh_ROpeaking_run_onset_run_bout_amp_violin'),
                                dpi=300)
    ...
    for ext in ['.png', '.pdf']:
        fig.savefig(
            BEHAVIOUR_OUTPUT_STEM / f'overall_matched_bouts{ext}',
            dpi=300,
            bbox_inches='tight'
            )
...
if __name__ == '__main__':
    analyse_identity(tagged_RO_peak_keys, 'tagged')
    analyse_identity(putative_RO_peak_keys, 'putative')
