...
txt_path = (
    ...
    / 'A063r-20230713'
    / 'A063r-20230713-01'
    / 'A063r-20230713-01T.txt'
)


#%% main
recname = txt_path.stem.removesuffix('T')
output_stem = pp.BEHAVIOUR_FIGURES_STEM / 'example_figures'
...
for ext in ['.png', '.pdf']:
    fig.savefig(output_stem / f'{recname}_spatial{ext}',
                dpi=300,
                bbox_inches='tight')
...
for ext in ['.png', '.pdf']:
    fig.savefig(output_stem / f'{recname}_temporal{ext}',
                dpi=300,
                bbox_inches='tight')
