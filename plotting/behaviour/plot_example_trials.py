...
from common_functions import replace_outlier, mpl_formatting
import project_paths as pp
mpl_formatting()


#%% path stems
example_stem = pp.BEHAVIOUR_FIGURES_STEM / 'trial_profiles' / 'example_trials'
...
#%% main
beh_path = pp.behaviour_experiment_stem(experiment) / f'{use_sess}.pkl'
with open(beh_path, 'rb') as f:
    beh = pickle.load(f)
session_example_stem = example_stem / use_sess
session_example_stem.mkdir(parents=True, exist_ok=True)
...
# save
for ext in ['.png', '.pdf']:
    fig.savefig(session_example_stem / f'{start_trial}_{end_trial}{ext}',
        dpi=300,
        bbox_inches='tight')
