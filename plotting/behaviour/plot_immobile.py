...
axs[1].set(xlim=(-pre_cue_time, max_trial_time), xlabel='time from cue (s)',
           ylim=(0, max(np.nanmax(mean_lick_profile), 1)),
           ylabel='lick rate (Hz)')
...
fig.suptitle(f'{recname}n{info}')
fig.tight_layout()

outpath = pp.BEHAVIOUR_FIGURES_STEM / 'immobile' / animal_folder
outpath.mkdir(parents=True, exist_ok=True)

fig.savefig(outpath / f'{recname}.png',
            dpi=300,
            bbox_inches='tight')
...
fig.savefig(outpath / f'{recname}_blackout_vs_firstlick.png',
            dpi=300, bbox_inches='tight')

fig, axs = plt.subplots(1,3, figsize=(6,2.5))

axs[0].hist(last_lick_aligned, bins=30, color='magenta', alpha=0.7)
axs[0].set_title(f'align to last licksnstd={np.std(last_lick_aligned):.2f}')

axs[1].hist(cue_aligned, bins=30, color='orange', alpha=0.7)
axs[1].set_title(f'align to cuenstd={np.std(cue_aligned):.2f}')

axs[2].hist(last_reward_aligned, bins=30, color='green', alpha=0.7)
axs[2].set_title(f'align to last rewardnstd={np.std(last_reward_aligned):.2f}')
...
fig.tight_layout()
fig.savefig(outpath / f'{recname}_alignment_histograms.png',
             dpi=300, bbox_inches='tight')


#%% plot example good trials
target_time = start_cue_duration + delay_duration
tolerance = 2  # s around the expected first-lick time
...
fig_height = max(len(example_trials)*.12, 1)
...
if len(example_trials) == 0:
    ax.set_ylim(0, 1)

ax.set(xlabel='time from cue (s)', ylabel='trial # (original index)',
       title='example good trials')
fig.tight_layout()

fig.savefig(outpath / f'{recname}_example_good_trials.png',
            dpi=300, bbox_inches='tight')
