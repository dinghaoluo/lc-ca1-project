...
Originally named plot_raw traces_axon_gcamp_example_trials.py
...
#%% parameters
path = pp.TWO_PHOTON_ROOT / 'A101i' / 'A101i-20241107' / 'A101i-20241107-03'
...
#%% beh
F = np.load(path / 'suite2p' / 'plane0' / 'F.npy', mmap_mode='r')

recname = path.name
beh_pkl = pp.behaviour_experiment_stem('LCHPCGCaMP') / f'{recname}.pkl'
...
save_dir = pp.LCHPC_AXON_FIGURES_STEM / 'raw_trace_distance_speed_examples'
save_dir.mkdir(parents=True, exist_ok=True)
outpath = save_dir / f'roi_{target_roi_id}_trials_{chosen[0]}_{chosen[-1]}'
