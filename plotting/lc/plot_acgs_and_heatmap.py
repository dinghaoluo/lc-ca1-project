...
single_cell_acg_stem = pp.LC_EPHYS_FIGURES_STEM / 'single_cell_ACGs'
population_acg_stem  = pp.LC_EPHYS_FIGURES_STEM / 'population_ACGs'
single_cell_acg_stem.mkdir(parents=True, exist_ok=True)
population_acg_stem.mkdir(parents=True, exist_ok=True)
...
for clu in cell_profiles.itertuples():
    ...
    for ext in ['.png', '.pdf']:
        fig.savefig(
            single_cell_acg_stem / f'{cluname} {identity}{ext}',
            dpi=300,
            bbox_inches='tight'
            )
...
for ext in ('.png', '.pdf'):
    fig.savefig(
        population_acg_stem / f'tagged_mean{ext}',
        dpi=300,
        bbox_inches='tight'
        )
...
for ext in ('.png', '.pdf'):
    fig.savefig(
        population_acg_stem / f'putative_mean{ext}',
        dpi=300,
        bbox_inches='tight'
        )
...
for ext in ('.png', '.pdf'):
    fig.savefig(
        population_acg_stem / f'other_mean{ext}',
        dpi=300,
        bbox_inches='tight'
        )
...
for ext in ('.png', '.pdf'):
    fig.savefig(population_acg_stem / f'tagged_ACGs{ext}',
                dpi=300,
                bbox_inches='tight')
plt.close(fig)
...
for ext in ('.png', '.pdf'):
    fig.savefig(population_acg_stem / f'putative_ACGs{ext}',
                dpi=300,
                bbox_inches='tight')
plt.close(fig)
...
for ext in ('.png', '.pdf'):
    fig.savefig(population_acg_stem / f'other_ACGs{ext}',
                dpi=300,
                bbox_inches='tight')
plt.close(fig)
