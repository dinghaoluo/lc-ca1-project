...
Originally named plot_neu_activity_on_off_mean_profile.py
...
from common_functions import normalise_to_all, mpl_formatting
...
warped_dict = np.load(pp.LC_EPHYS_STEM / 'LC_all_warped.npy', allow_pickle=True).item()
lick_sensitivity_stem = pp.LC_EPHYS_FIGURES_STEM / 'lick_sensitivity'
lick_sensitivity_stem.mkdir(parents=True, exist_ok=True)
...
for ext in ['.png', '.pdf']:
    fig.savefig(
        lick_sensitivity_stem / f'lick_sensitive_profiles_shuf_piechart{ext}',
        dpi=300,
        bbox_inches='tight'
        )
...
for ext in ['.png', '.pdf']:
    fig.savefig(
        lick_sensitivity_stem / f'lick_sensitive_profiles_shuf{ext}',
        dpi=300,
        bbox_inches='tight')
