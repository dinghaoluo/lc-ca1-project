...
Originally named plot_comp_tagged_putative.py
Modified on 28 Feb
...
from console_formatting import print_statistics_section
...
LC_figures_stem = pp.LC_EPHYS_FIGURES_STEM
(LC_figures_stem / 'UMAP' / 'summary_and_comparison').mkdir(parents=True, exist_ok=True)


#%% waveform and rate summaries
def _accumulate_info(df: pd.DataFrame,
    ...
    - method: str (default: 'FWHM')
        ...
        - 'FWHM': full width at half minimum (default, interpolated)
    ...
    for clu in df.itertuples():
        ...
        if method == 'half_width':
            ...
            baseline = np.median(wf_hi[:10])  # pre-trough baseline
        ...
        if method == 'half_width':
            ...
            # show the interpolated half-maximum level
            ax.axhline(half_max, linestyle='--', color='grey', label='half-max', zorder=1)

        ax.set(title=clu.Index)
        ax.legend(loc='best', fontsize='x-small')

        save_dir = LC_figures_stem / 'single_cell_waveform' / 'spike_width_calculation_{method}'
...
for ext in ('.png', '.pdf'):
    fig.savefig(
        LC_figures_stem / 'UMAP' / 'summary_and_comparison' / f'spike_width_v_spike_rate{ext}',
        dpi=300,
        bbox_inches='tight'
        )
...
for ext in ('.png', '.pdf'):
    fig.savefig(
        LC_figures_stem / 'UMAP' / 'summary_and_comparison' / f'spike_width_boxplot{ext}',
        dpi=300,
        bbox_inches='tight'
        )
...
for ext in ('.png', '.pdf'):
    fig.savefig(
        LC_figures_stem / 'UMAP' / 'summary_and_comparison' / f'spike_rate_logged_boxplot{ext}',
        dpi=300,
        bbox_inches='tight'
        )
