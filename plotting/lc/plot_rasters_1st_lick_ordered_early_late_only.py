...
output_stem = pp.LC_EPHYS_FIGURES_STEM / 'lick_sensitivity' / 'rasters_by_first_licks_earlyvlate_only'
...
for cluname in clu_list:
    ...
    rasters = load_lc_session_dict(cluname[:17], 'all_rasters')[cluname]
    trains = load_lc_session_dict(cluname[:17], 'all_trains')[cluname]
    ...
    stim_idx = np.where(stim_on!=0)[0]+1  # +1 for old MATLAB indexing problem
    ...
    for ext in ['.png', '.pdf']:
        fig.savefig(
            output_stem / f'{cluname} {identity}{ext}',
            dpi=300,
            bbox_inches='tight')
