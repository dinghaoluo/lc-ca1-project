...
#%% load data
all_info = np.load(pp.LC_ALL_TAGGED_STEM / 'LC_all_tagged_info.npy',
...
beh_par_file = sio.loadmat(f'{filename}_behPar_msess1.mat')
...
plt.show()
for ext in ['png','pdf']:
    fig.savefig(pp.LC_ALL_TAGGED_FIGURES_STEM / f'LC_tagged_egcell_goodvbad_raster.{ext}',
...
plt.show()
for ext in ['png','pdf']:
    fig.savefig(pp.LC_ALL_TAGGED_FIGURES_STEM / f'LC_tagged_egcell_goodvbad.{ext}',
                dpi=200, bbox_inches='tight')
plt.close(fig)
