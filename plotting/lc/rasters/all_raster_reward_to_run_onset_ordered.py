...
Originally named all_raster_rew_to_run_ordered.py
...
output_no_stim = pp.LC_ALL_FIGURES_STEM / 'single_cell_raster_by_rew2run_noStim'
output_stim = pp.LC_ALL_FIGURES_STEM / 'single_cell_raster_by_rew2run'
...
for cluname in clu_list:
    ...
    temp_ordered = sorted(trial_list, key=lambda x: starts[x] - pumps[x-1][0])
    rew2run_ordered = [pumps[t][0] for t in temp_ordered]
    ...
    for trial in range(tot_trial-1):
        ...
        window = [rew2run_ordered[trial]+3750-625, rew2run_ordered[trial]+3750, rew2run_ordered[trial]+3750+625]
        ...
        axs['A'].plot([rew2run_ordered[trial]/1250, rew2run_ordered[trial]/1250],
                      [trial, trial+1],
    ...
    rew_line, = axs['A'].plot([],[],color='darkgreen',label='rew.')
    fl, = axs['A'].plot([],[],color='orchid',alpha=.35,label='1st licks')
    axs['A'].legend(handles=[fl, rew_line], frameon=False, fontsize=8)
...
#%% save to dataframe
cell_prop = cell_prop.assign(lick_sensitive=pd.Series(last_rew_sensitive).values)
cell_prop = cell_prop.assign(lick_sensitive_type=pd.Series(last_rew_sensitive_type).values)
