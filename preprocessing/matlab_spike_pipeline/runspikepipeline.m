function RunSpikePipeline(filename)
    ...
    if nargin < 1 || isempty(filename)
        ...
        paths = listRecordingsActiveLickPath(1:end, :);
        files = listRecordingsActiveLickFileName(1:end, :);
        tot_rec = size(paths, 1);
    else
        filename = strtrim(char(filename));
        suffix_idx = strfind(filename, '_DataStructure');
        if ~isempty(suffix_idx)
            filename = filename(1:suffix_idx(1)-1);
        end
        paths = {fullfile('Z:\Dinghao\MiceExp', ['ANMD' filename(2:5)], filename(1:14), filename)};
        files = {filename};
        tot_rec = 1;
    end
    ...
    for i = 1:tot_rec
        ...
        GenerateBehavElectroDataStructures_smTrMPFIv2_opto(output_parent_dir, filename);
        ...
        % tagging-related functions are kept for non-pixel recordings
