function BasicInfo_smTr(path,fileName)
...
% e.g.: BasicInfo_smTr('./','A111-20150301-01_DataStructure_mazeSection1_TrialType1')
    ...
    if(exist('cluList','var') && ~isempty(cluList))
        %%%%%%%%% mahala distance and refractory period
        rec.mahalDist = cluList.mahalDist;
        rec.refracViolPercent = cluList.refracViolPercent;
        rec.firingRate = cluList.firingRate;
        
        % number of neurons
        rec.numNeurons = length(cluList.firingRate); 
        
        % whether a neuron is an interneuron based on its waveshape
        rec.isIntern = cluList.isIntern;
    else
        %%%%%%%%% mahala distance and refractory period
        rec.mahalDist = -1;
        rec.refracViolPercent = -1;
        rec.firingRate = -1;
        
        rec.numNeurons = 0;
        
        rec.isIntern = []; % whether a neuron is an interneuron based on its waveshape
    end
    ...
    beh.pauseBefTrial = zeros(1,beh.numTrials); 
    beh.pauseWithinTrial = zeros(1,beh.numTrials); 
    beh.indGoodLap = [];
    beh.indErrLap = [];
    beh.indCorrLap = [];
    ...
    beh.thetaFreqHMean = zeros(1,beh.numTrials);
    ...
    for i = 1:beh.numTrials
        if(lapList(i,1) ~= -1 && ~isempty(trials{i}))
            ...
            beh.pauseWithinTrial(i) = (beh.lenTrials(i) - ...
                sum(beh.indRunInLap{i}))/sampleFq;
            beh.thetaFreqHMean(i) = mean(trials{i}.thetaFreqHilb(beh.indRunInLap{i}));
...
%             end
            ...
            if(strcmp(fileName(1:4),'A011') || ...
                    strcmp(fileName(1:4),'A012'))
                if(~isempty(trials{i}))
                    beh.indCorrLap = [beh.indCorrLap i]; 
                else
                    beh.indErrLap = [beh.indErrLap i]; 
                end
            else
                %%%% added by Yingxue on 4/11/2019
                if(beh.mazeType(i) >= 12 && beh.mazeType(i) <= 15)
                    indLick = trials{i}.xMM(trials{i}.lickLfpInd) > ...
                        beh.trackLen(i)/2;
                    if(sum(indLick) > 0 && lap.trStartCueLR(i) == 0)
                        %%% right trial (no-go) with lick
                        beh.indErrLap = [beh.indErrLap i];
                    elseif(sum(indLick) == 0 && lap.trStartCueLR(i) == 1)
                        beh.indErrLap = [beh.indErrLap i];
                    else
                        beh.indCorrLap = [beh.indCorrLap i]; 
                    end
...
%         pause;
    ...
    if(isfield(lap,'trStartCueLR'))
        beh.trStartCueLR = lap.trStartCueLR;
    end
                  
    %%%%%%%%% stimulation experiments
%     beh.indLapStim = [];
%     for i = 1:beh.numTrials
%         if(~isempty(trials{i}) && trials{i}.isStimulated == 1) 
%             % if the experiment contains stimulation
%             beh.indLapStim = [beh.indLapStim i];
%         end
%     end
