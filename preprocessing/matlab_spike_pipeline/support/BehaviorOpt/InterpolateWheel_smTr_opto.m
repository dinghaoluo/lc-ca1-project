function InterpolateWheel_smTr_opto(file,lfpFreq)
...
if exist([file '-whl.mat'], 'file') == 2
    ...
    for i = 1:length(wheelTimeLfp)-1
        if(wheelDist(i+1) >= wheelDist(i))
            ...
            interpTspDistLfp1(round(wheelTimeLfp(i)-behEventsTdt.taskDescr(1,1)+1):...
    ...
    for tr = 1:(max(behEventsTdt.trialDescr(:,3)))
        ...
        % changed on 2/16/2019 for the active licking task
