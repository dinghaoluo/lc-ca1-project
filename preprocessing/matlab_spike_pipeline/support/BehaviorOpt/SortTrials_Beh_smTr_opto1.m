...
%     end
    ...
    Laps.stimOffLfpExp = zeros(totNLaps,1);
    Laps.stimOffLfpExact = zeros(totNLaps,1);
    ...
    %% added by Yingxue on 1/31/2022
    if(isfield(behEventsTdt,'stimOff'))
        lenStimOff = length(behEventsTdt.stimOff);
        lenStimOn = size(behEventsTdt.stimOn,1);
        if(lenStimOn ~= lenStimOff)
            disp('WARNING: stimOn and stimOff have different length');
            if(length(behEventsTdt.stimOff) > length(behEventsTdt.stimOn(:,1)))
                stimOff = zeros(lenStimOn,1);
                numPulses = zeros(lenStimOn,1);
                startStim = 1;
                for i = 1:lenStimOn
                    diffOnOffStim = behEventsTdt.stimOff - behEventsTdt.stimOn(i,1);
                    indOff = find(diffOnOffStim >= 0, 1,'first');
                    if(diffOnOffStim(indOff) <= behEventsTdt.stimOn(i,4)*lfpFreq/1000*2.5) % pulse width 
                        % find the corresponding stimOff time for each stimOn  
                        % the pulse width does not have a lower bound,
                        % because it can be turned off at any time
                        stimOff(i) = behEventsTdt.stimOff(indOff);
                    else
                        test = 1;
                    end
                    if(i > 1)
                        if(behEventsTdt.stimOn(i,1) - behEventsTdt.stimOn(i-1,1) > ...
                           behEventsTdt.stimOn(i,5)*lfpFreq/1000*10) 
                        % find the real number of stim pulses for each stim
                        % pulse
                            numPulses(startStim:i-1) = i - startStim;
                            startStim = i;
                        end
                    end
                end
                numPulses(startStim:end) = lenStimOn - startStim + 1;
                behEventsTdt.stimOff = stimOff;
                behEventsTdt.stimOn(:,3) = numPulses;
            end
        end
    end
...
%       figure(10); clf;      
        ...
        if(isfield(behEventsTdt,'stimOn'))
            ...
            if(Laps.mazeType(nTr) ~= 0 && sum(ind) == 0)
                Laps.mazeType(nTr) = 0;
                disp(['Trial' num2str(nTr) ' is supposed to be a stimulation trial,' ...
                            'but no stimulation pulses is available.']);
            end
            if(sum(ind) > 0)
...
%                         length(behEventsTdt.stimOff(:,1)) == length(behEventsTdt.stimOn(:,1))
                ...
                %% changed by Yingxue on 1/25/2022
                if(Laps.mazeType(nTr) ~= 0) % stimulation trial
                    indStim = find(ind == 1,1);
                
                    %% calcuate total pulse width
                    if(behEventsTdt.stimOn(indStim,3) == 1 && behEventsTdt.stimOn(indStim,7) <= 1)
                        totPulseWidth = resamp(behEventsTdt.stimOn(indStim,4),lfpFreq);
                        if(isfield(behEventsTdt,'stimOff'))
                            Laps.stimOffLfpExact(nTr) = ...
                                behEventsTdt.stimOff(indStim,1) - behEventsTdt.taskDescr(1,1) + 1;
                        else
                            Laps.stimOffLfpExact(nTr) = ...
                                behEventsTdt.stimOn(indStim,1) + totPulseWidth ...
                                - behEventsTdt.taskDescr(1,1) + 1;
                        end
                    elseif(behEventsTdt.stimOn(indStim,3) > 1 && ...
                            behEventsTdt.stimOn(indStim,7) <= 1) % changed on 1/1/2021 by Yingxue from == 1 to <= 1
                        totPulseWidth = behEventsTdt.stimOn(indStim,5)*...
                            (behEventsTdt.stimOn(indStim,3) - 1) + ...
                            behEventsTdt.stimOn(indStim,4);
                        totPulseWidth = resamp(totPulseWidth,lfpFreq);
                        Laps.stimOffLfpExact(nTr) = ...
                             behEventsTdt.stimOff(indStim+behEventsTdt.stimOn(indStim,3)-1,1) - ...
                              behEventsTdt.taskDescr(1,1) + 1;
                    else
                        totPulseWidth = (behEventsTdt.stimOn(indStim,5)*...
                            behEventsTdt.stimOn(indStim,3) + behEventsTdt.stimOn(indStim,6)) * ...
                            behEventsTdt.stimOn(indStim,7) - behEventsTdt.stimOn(indStim,6) - ...
                            (behEventsTdt.stimOn(indStim,5) + behEventsTdt.stimOn(indStim,4));
                        totPulseWidth = resamp(totPulseWidth,lfpFreq);
                        Laps.stimOffLfpExact(nTr) = ...
                             behEventsTdt.stimOff(indStim + ...
                             behEventsTdt.stimOn(indStim,3) * behEventsTdt.stimOn(indStim,7)-1,1) + ...
                             behEventsTdt.stimOn(indStim,4)*lfpFreq/1000 - behEventsTdt.taskDescr(1,1) + 1;
                    end

                    if((pulseMethod >= 1 && pulseMethod <= 4) || pulseMethod == 7 || pulseMethod == 8) % start cue, run onset, mid-cue, and water delivery
                        Laps.stimOffLfpExp(nTr) = behEventsTdt.stimOn(indStim,1) ...
                            - behEventsTdt.taskDescr(1,1) + 1 ...
                            + totPulseWidth - 1;  
                    elseif(pulseMethod == 5)
                        Laps.stimOffLfpExp(nTr) = behEventsTdt.stimOn(indStim,1) ...
                            - behEventsTdt.taskDescr(1,1) + 1 ...
                            + totPulseWidth - 1;
                        if(Laps.stimOffLfpExp(nTr) > trEndLfpInd)
                            Laps.stimOffLfpExp(nTr) = trEndLfpInd;
                        end
                    elseif(pulseMethod == 6)
                        ...
                        if(nTr < totNLaps)
                            ...
                            if(sum(wheelEventsCur(end-Thre_time:end,2) > Thre_speed) > Thre_time)
                                Laps.stimOffLfpExp(nTr) = trStartLfpIndArr(nTr+1);
                            else
                                ...
                                if((stopTime - behEventsTdt.stimOn(indStim,1) + 1) ...
                                        < totPulseWidth)
                                    Laps.stimOffLfpExp(nTr) = stopTime - behEventsTdt.taskDescr(1,1) + 1;
                                end
                            end
                        end
                    end
                ...
                for n = 1:length(indStim)
                    ...
                    elseif(isfield(behEventsTdt,'stimOff') && ... % added by Yingxue on 1/1/2021, to adjust for stim condition 7 and 8
                        ...
                        disp(['Tr = ' num2str(nTr) ' OnT = ' num2str(stimOnLfp) ' OffT = ' ...
                            num2str(Laps.stimOffLfpExp(nTr))]);
                        Track.stimOn(stimOnLfp:....
                            Laps.stimOffLfpExact(nTr)) = 1;
                    end
                end
            end
        else
