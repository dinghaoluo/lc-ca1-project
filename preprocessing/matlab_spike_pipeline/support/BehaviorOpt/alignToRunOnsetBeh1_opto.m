function alignToRunOnsetBeh1_opto(path, fileName, mazeSess)
% align each trial to the running onset
    ...
    minSpeed1 = 100;
    minSpeed = 10; % changed on 7/9/2020
    smoothSpan = 100;
    nSampBef = 3*sampleFq;
    trialsRun = [];
    ...
    for i = 1:length(indTr)
        ...
        trialsRun.goodTrial(i) = indStart;
        trialsRun.mazeType(i) = Laps.mazeType(indTr(i));
        trialsRun.mazeTypeMod(i) = Laps.mazeTypeMod(indTr(i));
        ...
        trialsRun.stimOnLfpInd{i} = Laps.stimOnLfpInd{indTr(i)};
        trialsRun.stimOffLfpInd{i} = Laps.stimOffLfpInd{indTr(i)};
        trialsRun.stimOffLfpIndExp(i) = Laps.stimOffLfpExp(indTr(i)); % changed on 1/25/2022
        trialsRun.stimOffLfpIndExact(i) = Laps.stimOffLfpExact(indTr(i)); % changed on 1/25/2022
        trialsRun.stimTotNumPulse{i} = Laps.stimTotNumPulse{indTr(i)};
        trialsRun.stimNumPulse{i} = Laps.stimNumPulse{indTr(i)};
        trialsRun.stimPulseWidth{i} = Laps.stimPulseWidth{indTr(i)};
        trialsRun.stimPulsePeriod{i} = Laps.stimPulsePeriod{indTr(i)};
        trialsRun.stimOffTime{i} = Laps.stimOffTime{indTr(i)};
        trialsRun.stimRepeats{i} = Laps.stimRepeats{indTr(i)};
        trialsRun.stimPulseMethod(i) = Laps.stimPulseMethod{indTr(i)};
        ...
        %% check whether trialsRun.startLfpInd(i)-nSampBef < 0
        indTmp = indEegBef <= 0;
        numLessThanZero = sum(indTmp);
        if(numLessThanZero > 0)
            trialsRun.speed_MMsecBef{i} = zeros(nSampBef,1);
            trialsRun.speed_MMsecBef{i}(numLessThanZero+1:numLessThanZero+trialsRun.startLfpInd(i)-1) = ...
                Track.speed_MMsecAll(1:trialsRun.startLfpInd(i)-1);
            
        else
            trialsRun.speed_MMsecBef{i} = Track.speed_MMsecAll(indEegBef);
        end
        %% changed on 1/7/2022 by Yingxue
        
%         figure(1);
%         plot(Track.speed_MMsecAll(trialsRun.startLfpInd(i):trialsRun.endLfpInd(i)))
%         set(gca,'YLim',[0 800]);
%         pause;
