function PyrSpModAllRec(onlyRun,taskSel,methodKMean)
...
%     disp('No cue')
%     spModPyrNoCue = accumPyrNeurons1(listRecordingsNoCuePath,...
...
function modPyr = accumPyrNeurons1(paths,filenames,mazeSess,minFR,maxFR,task,methodTheta,onlyRun)
    ...
    for i = 1:numRec
        ...
        if(exist(fullPath) == 0)
            disp(['The field detection file does not exist. Please call ',...
                    'function "_SpInfo_GoodTr_Run" first.']);
            return;
        end
