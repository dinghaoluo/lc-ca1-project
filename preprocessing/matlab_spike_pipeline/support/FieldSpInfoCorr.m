function fieldStruct = FieldSpInfoCorr(pFRStruct,spatialInfo,meanCorr,paramF)
% Firing field detection based on correlation distance and spatial information
    ...
    fieldStruct.indNeuron = find(pFRStruct.meanInstFR > paramF.minInstFR & ...
        (spatialInfo >= paramF.minSpInfo & meanCorr.meanNZ >= paramF.minCorr) & ...
        fieldStruct.numTrials >= paramF.minNumTr & ...
        meanCorr.nNonZeroTr >= paramF.percNumActiveTr*fieldStruct.numTrials);
    ...
    fieldStruct.meanCorrNZ = meanCorr.meanNZ(fieldStruct.indNeuron);
    fieldStruct.numActiveTrials = meanCorr.nNonZeroTr(fieldStruct.indNeuron);
    
%     if(sum(fieldStruct.indNeuron == 54) == 1)
%         a = 1;
%     end
    ...
    for i = 1:length(fieldStruct.indNeuron)
        ...
        if(fieldStruct.indNeuron(i) == 24 || fieldStruct.indNeuron(i) == 31 || fieldStruct.indNeuron(i) == 46)
            a = 1;
        end
...
%         if(fieldStruct.indEndField(i) < size(pFRStruct.avgFRProfile,2))
%             indAfterFieldEnd = min(fieldStruct.indEndField(i)+paramF.reboundCheckRegion,size(pFRStruct.avgFRProfile,2));
...
%         hold off
    ...
    ind = find(fieldStruct.FW < paramF.maxFieldWidth & indBadField == 0);
