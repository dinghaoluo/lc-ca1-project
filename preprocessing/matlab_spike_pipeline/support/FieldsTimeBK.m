...
for i = 1:numNeurons
    ...
    if(meanInstFR(indNeurons(i)) < paramFW.highInstFR)
        ...
        paramFW.percSpikesInFields = paramFW.percSpikesInFieldsHigh;
    end
    ...
    if(meanInstFR(indNeurons(i)) < paramFW.highInstFR) 
        indexHighFR = find(avgFRProfile(indNeurons(i),:) >= ...
            peakFR(indNeurons(i))*paramFW.pToMFRRatioFieldMinLowFR); 
    else
        indexHighFR = find(avgFRProfile(indNeurons(i),:) >= ...
            peakFR(indNeurons(i))*paramFW.pToMFRRatioFieldMinHighFR); 
    end
                        ...
                        for k = 1:numFieldNew
                            ...
                            for m = 1:fieldStruct.numTrials
                                spikePerNeuronPerTrial = ...
                                    spikesPerNeuPerTr{indNeurons(i),m};
                                numSpikePerTrialPerField(m) = ...
                                    length(find(spikePerNeuronPerTrial >= ...
                                            paramGen.timeSteps(indATmp(k))...
                                        & spikePerNeuronPerTrial <= ...
                                            paramGen.timeSteps(indBTmp(k))));
...
%                             end
                            if(numActTrial >= ...
                                ...
                                percSpikeIn = ...
                                    numSpikesPerField...
                                    /length(spTimePerNeuron{indNeurons(i)});
...
%                         end
                        ...
                        if(numSpikesInField/(distSpikesInField(end)-distSpikesInField(1))...
                            >= paramFW.minSpikeDenPerFieldPerHz*numActTrial)
