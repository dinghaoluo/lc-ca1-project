function ThetaModulation_GoodTr(path,fileName,onlyRun,figState)
...
% by Yingxue 08/25/2017
    ...
    fileNameThetaMod = [fileName '_ThetaMod_GoodTr_Run' num2str(onlyRun) '.mat'];
    ...
    thetaMod = struct('ACGSpectrum',[],... % ACG spectrum
                      ...
                      'filteredACG',zeros(totClu,length(CCGSessGoodTr{1}.ccgT)),... % smoothed ACG
    ...
    thetaModSessGoodTr = cell(1,length(mazeSess));
    thetaModSessBadTr = cell(1,length(mazeSess));
    thetaModSessOKTr = cell(1,length(mazeSess));
    for i = 1:length(mazeSess)
        disp(['Session ' num2str(i)]);
        if(~isempty(CCGSessGoodTr{i}))
            thetaModSessGoodTr{i} = thetaMod;
            thetaModSessGoodTr{i} = calThetaMod(totClu,thetaModSessGoodTr{i},CCGSessGoodTr{i},paramT);
        end
        if(~isempty(CCGSessBadTr{i}))
            thetaModSessBadTr{i} = thetaMod;
            thetaModSessBadTr{i} = calThetaMod(totClu,thetaModSessBadTr{i},CCGSessBadTr{i},paramT);
        end
        if(~isempty(CCGSessOKTr{i}))
            thetaModSessOKTr{i} = thetaMod;
            thetaModSessOKTr{i} = calThetaMod(totClu,thetaModSessOKTr{i},CCGSessOKTr{i},paramT);
        end
    end
    
    fullPath = [path fileNameThetaMod];
    save(fullPath, 'thetaModSessGoodTr','thetaModSessBadTr',...
        'thetaModSessOKTr','paramT');
    ...
    if(figState ~= 0)
        ...
        for i = 1:numSess
            thetaMod = thetaModSessGoodTr{i}.thetaMod(exc);
