function PyrInitPeakAllRecSigAllNoStimStimCtrl()
            
    RecordingListPyrIntSST;  % include all the early NoCue (no blackout), PL and AL (up to A057)
    pathAnal0 = 'Z:\Yingxue\Draft\SST\Pyramidal\';
    
    if(exist([pathAnal0 'initPeakPyrAllRecSigAllNoStimStimCtrl.mat']))
        load([pathAnal0 'initPeakPyrAllRecSigAllNoStimStimCtrl.mat']);
    end
    pathAnalPeak0 = 'Z:\Yingxue\Draft\SST\PyramidalInitPeakALPL\2\';
    ...
    % if(exist('modPyr1SigNoCue') == 0)
        ...
        fullpath = [pathAnal0 'initPeakPyrAllRecSigAllNoStimStimCtrl.mat'];
...
function modPyr1Sig = ...
    ...
    for i = 1:numRec
        ...
        load(fullPath,'trialNoNonStimGood','trialNoNonStimBad','trialNoStimCtrl');
        ...
        trialNoStimCtrlAll = [];
        if(~isempty(trialNoStimCtrl))
            for j = 1:length(trialNoStimCtrl)
                trialNoStimCtrlAll = [trialNoStimCtrlAll; trialNoStimCtrl{j}];
            end
        end
        trialNoNonStimGood = [trialNoNonStimGood; trialNoNonStimBad; trialNoStimCtrlAll];
