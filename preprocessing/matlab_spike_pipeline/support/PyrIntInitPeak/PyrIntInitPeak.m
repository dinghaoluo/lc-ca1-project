...
% compare Pyramidal neurons and PV interneurons on their initial peak
    ...
    pathAnal0 = 'Z:\Yingxue\Draft\SST\Pyramidal\';
    pathAnalInt0 = 'Z:\Yingxue\Draft\SST\Interneuron\';
    ...
    pathAnalPeak0 = 'Z:\Yingxue\Draft\SST\PyramidalInitPeak\2\';
    ...
    if(taskSel == 1) % including all the neurons
        pathAnal = 'Z:\Yingxue\Draft\SST\PyramidalIntInitPeak\';
    elseif(taskSel == 2) % including AL and PL neurons
        pathAnal = 'Z:\Yingxue\Draft\SST\PyramidalIntInitPeakALPL\';
    else
        pathAnal = 'Z:\Yingxue\Draft\SST\PyramidalIntInitPeakAL\';
    end
    ...
    %     idx1 = find(ratio0to1BefRunOrd >= 0.5,1,'last');
        ...
        isNeuWithFieldAligned.isFieldRiseBad = InitAll.isNeuWithFieldAligned(PyrRise.idxRiseBad);
        isNeuWithFieldAligned.isFieldDownBad = InitAll.isNeuWithFieldAligned(PyrDown.idxDownBad);
        isNeuWithFieldAligned.isFieldOtherBad = InitAll.isNeuWithFieldAligned(PyrOther.idxOtherBad); % added on 7/8/2022
        ...
        FRProfileMeanPyr.RiseFieldBad = accumMeanPVStim(InitAll.avgFRProfileBad(isNeuWithFieldAligned.idxRiseBadBad,:),...
            modPyr1NoCue.timeStepRun);
        FRProfileMeanPyr.DownFieldBad = accumMeanPVStim(InitAll.avgFRProfileBad(isNeuWithFieldAligned.idxDownBadBad,:),...
            modPyr1NoCue.timeStepRun);
        FRProfileMeanPyr.OtherFieldBad = accumMeanPVStim(InitAll.avgFRProfileBad(isNeuWithFieldAligned.idxOtherBadBad,:),...
            modPyr1NoCue.timeStepRun); % added on 7/8/2022
    ...
    %         FRProfileMeanIntStat.DownGoodBad{i} = accumMeanPyrIntInitPeakStatGoodBad(FRProfileMeanInt.Down{i},FRProfileMeanInt.DownBad{i});
        ...
        save([pathAnal 'initPeakPyrIntAllRec.mat'],'InitAll','PyrRise','PyrDown','PyrOther',...
            ...
            'relDepthNeuHDefPyr','isNeuWithFieldAligned'); 
    end
    
    % PyrRise neurons 50% ramping time
    time50percAvgFRPyrRise = findHalfWayRiseTime(InitAll.avgFRProfile(PyrRise.idxRise,:),modPyr1NoCue.timeStepRun,1,0);
    
    % PyrDown neurons 50% ramping time
    time50percAvgFRPyrDown = findHalfWayRiseTime(InitAll.avgFRProfile(PyrDown.idxDown,:),modPyr1NoCue.timeStepRun,2,0);
    
    %% for all the interneurons
    time50percAvgFRIntRise = cell(1,max(InitAll.idxC2Int));
    pTime50percAvgFR.PyrRiseVsIntRise = zeros(1,max(InitAll.idxC2Int));
    pTime50percAvgFR.PyrDownVsIntRise = zeros(1,max(InitAll.idxC2Int));
    for i = 1:max(InitAll.idxC2Int)
        time50percAvgFRIntRise{i} = findHalfWayRiseTime(InitAll.avgFRProfileInt(IntRise.idxRise{i},:),modPyr1NoCue.timeStepRun,1,0);
        pTime50percAvgFR.PyrRiseVsIntRise(i) = ranksum(time50percAvgFRPyrRise,time50percAvgFRIntRise{i});
        pTime50percAvgFR.PyrDownVsIntRise(i) = ranksum(time50percAvgFRPyrDown,time50percAvgFRIntRise{i});
        pTime50percAvgFR.meanIntRise(i) = mean(time50percAvgFRIntRise{i});
        pTime50percAvgFR.semIntRise(i) = std(time50percAvgFRIntRise{i})/sqrt(length(time50percAvgFRIntRise{i}));
    end
    pTime50percAvgFR.meanPyrRise = mean(time50percAvgFRPyrRise);
    pTime50percAvgFR.meanPyrDown = mean(time50percAvgFRPyrDown);
    pTime50percAvgFR.semPyrRise = std(time50percAvgFRPyrRise)/sqrt(length(time50percAvgFRPyrRise));
    pTime50percAvgFR.semPyrDown = std(time50percAvgFRPyrDown)/sqrt(length(time50percAvgFRPyrDown));
       
    save([pathAnal 'initPeakPyrIntAllRec50PercTime.mat'],'time50percAvgFRIntRise','time50percAvgFRPyrRise','time50percAvgFRPyrDown','pTime50percAvgFR');
...
%     %% plot depth of rise and down pyramidal neurons
    ...
    plotPyrNeuRiseDownField(pathAnal,modInt1NoCue.timeStepRun,InitAll,...
    ...
    plotIntNeuRiseDown(pathAnal,modInt1NoCue.timeStepRun,...
            InitAll,IntRise,IntDown,FRProfileMean,FRProfileMeanInt,FRProfileMeanIntStat,colorSel,[{'Good'} {'Bad'}]);
    close all;
    
    ComparePyrRiseDownInt(pathAnal,modInt1NoCue.timeStepRun,InitAll,PyrRise,PyrDown,IntRise);    
    close all;
end
