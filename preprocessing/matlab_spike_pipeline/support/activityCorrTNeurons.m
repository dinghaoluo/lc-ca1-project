function activityCorrTNeurons(path,fileName,timeBin,onlyRun,mazeSess)
...
%         indTr = indTr(mazeSessTr == mazeSess(1)); 
        ...
        activityCorrT{sess}.indPyrNeu = indPyrNeu;
        activityCorrT{sess}.indLaps = indTr;
        activityCorrT{sess}.corrMatxPerNeu = [];
        activityCorrT{sess}.meanCorrActiveTrPerNeu = zeros(1,length(indPyrNeu));
        activityCorrT{sess}.meanCorrAllTrPerNeu = zeros(1,length(indPyrNeu));
        activityCorrT{sess}.activeTr = zeros(1,length(indPyrNeu));
        for n = 1:length(indPyrNeu)
            corrMatxPerNeu = ...
                zeros(length(indTr),length(indTr));
            filteredSpikeArrayTmp = zeros(length(indTr),...
                size(filteredSpikeArray{indTr(1)},2));
            for i = 1:length(indTr)
                disp(['i = ' num2str(i) ' n = ' num2str(n)]);
                filteredSpikeArrayTmp(i,:) = ...
                    filteredSpikeArray{indTr(i)}(indPyrNeu(n),:);
            end
            ...
            activityCorrT{sess}.corrMatxPerNeu{n} = corrMatxPerNeu;
            activityCorrT{sess}.activeTr(n) = activeTr;
            activityCorrT{sess}.meanCorrActiveTrPerNeu(n) = ...
                sum(sum(triu(corrMatxPerNeu,1)))/count;
            activityCorrT{sess}.meanCorrAllTrPerNeu(n) = ...
                sum(sum(triu(corrMatxPerNeu,1)))/countAll;
        end
        indGoodNeu = activityCorrT{sess}.meanCorrActiveTrPerNeu >= 0.01 & ...
            activityCorrT{sess}.activeTr/length(indTr) > 0.4;
        activityCorrT{sess}.indGoodPyrNeu = indPyrNeu(indGoodNeu);
        
        figure;
        hist(activityCorrT{sess}.meanCorrActiveTrPerNeu,-0.025:0.01:0.4);
        xlabel('neuron activity corr. across trials')
        ylabel('count')
        title(fileName(1:13))
        if(sum(activityCorrT{sess}.meanCorrActiveTrPerNeu > 0.4) > 0)
            disp('There are neurons with correlation > 0.4');
        end
    end
        
    fileNameSpInfo = [fileName '_NeuActivityCorrT_Run' num2str(onlyRun) '.mat'];
    fileNameFull = [path fileNameSpInfo];
    save(fileNameFull,'activityCorrT');
end
