function PeakFiringRate_AlignedCueOff(path,fileName,fileState,onlyRun,mazeSess,figureState)
...
% PeakFiringRateVR('./','A111-20150301-01_DataStructure_mazeSection1_TrialType1',1,1,1,1)
    ...
    fileNamePeakFR = [fileName '_PeakFR_msess' num2str(mazeSess) '_CueOffRun' num2str(onlyRun) '.mat'];        
    if(fileState == 0)
        fileNameFull = [fileName '.mat'];
    else
        fileNameFull = [fileName '_convSpikesAligned_msess' num2str(mazeSess) '_BefCueOffRun' num2str(onlyRun) '.mat'];
    end
    ...
    if(isfield(beh,'indStimLap'))
        ...
        trialNoNonStimGood = find(beh.indStimLap(ind) == 0 & behPar.indTrBadBeh == 0 ...
            & behPar.numSamplesCueOff ~= 0);
        trialNoNonStimBad = find(beh.indStimLap(ind) == 0 & behPar.indTrBadBeh == 1 ...
            & behPar.numSamplesCueOff ~= 0);
        trialNoStim = find(beh.indStimLap(ind) == 1 ...
            & behPar.numSamplesCueOff ~= 0);
    else
        trialNoNonStimGood = find(behPar.indTrBadBeh == 0 & behPar.numSamplesCueOff ~= 0);
        trialNoNonStimBad = find(behPar.indTrBadBeh == 1 & behPar.numSamplesCueOff ~= 0);
        trialNoStim = [];
    end
     
    %%%%%%%%%% calculate the peak firing rate
    neuronNo = length(filteredSpikeArrayCueOffset);
    numSamples = length(timeStepRun);
    disp('calculate peak firing rate for non-stimulated good trials');
    pFRNonStimGoodStruct = PeakFRAligned(filteredSpikeArrayCueOffset,trialNoNonStimGood,neuronNo,numSamples);
    disp('calculate peak firing rate for non-stimulated bad trials');
    pFRNonStimBadStruct = PeakFRAligned(filteredSpikeArrayCueOffset,trialNoNonStimBad,neuronNo,numSamples);
    disp('calculate peak firing rate for stimulated trials');
    if(~isempty(trialNoStim))
        pFRStimStruct = PeakFRAligned(filteredSpikeArrayCueOffset,trialNoStim,neuronNo,numSamples);
    else
        pFRStimStruct = [];
    end
        
    save([path fileNamePeakFR], 'pFRNonStimGoodStruct','pFRNonStimBadStruct','pFRStimStruct','-v7.3');
