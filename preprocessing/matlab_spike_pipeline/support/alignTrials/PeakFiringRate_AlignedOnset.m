function PeakFiringRate_AlignedOnset(path,fileName,fileState,onlyRun,mazeSess,condition,figureState)
...
%
% Example: 
% PeakFiringRateVR('./','A111-20150301-01_DataStructure_mazeSection1_TrialType1',1,1,1,1,1)
    ...
    elseif(condition == 2)
        fileNamePeakFR = [fileName '_PeakFR_msess' num2str(mazeSess) '_RewOnset' num2str(onlyRun) '.mat'];   
        if(fileState == 0)
            fileNameFull = [fileName '.mat'];
        else
            fileNameFull = [fileName '_convSpikesAligned_msess' num2str(mazeSess) '_BefRew' num2str(onlyRun) '.mat'];
        end
    elseif(condition == 3)
        fileNamePeakFR = [fileName '_PeakFR_msess' num2str(mazeSess) '_CueOnset' num2str(onlyRun) '.mat'];   
        if(fileState == 0)
            fileNameFull = [fileName '.mat'];
        else
            fileNameFull = [fileName '_convSpikesAligned_msess' num2str(mazeSess) '_BefCue' num2str(onlyRun) '.mat'];
        end
    end
    %%
    ...
    if(condition == 1)
        ...
        filteredSpikeArrayOnSet = filteredSpikeArrayRunOnSet;
    else
        neuronNo = length(filteredSpikeArrayRewOnSet);
        filteredSpikeArrayOnSet = filteredSpikeArrayRewOnSet;
    end
    %%
    numSamples = length(timeStepRun);
    disp('calculate peak firing rate for non-stimulated good trials');
    pFRNonStimGoodStruct = PeakFRAligned(filteredSpikeArrayOnSet,trialNoNonStimGood,neuronNo,numSamples);
    disp('calculate peak firing rate for non-stimulated bad trials');
    pFRNonStimBadStruct = PeakFRAligned(filteredSpikeArrayOnSet,trialNoNonStimBad,neuronNo,numSamples);
    ...
    for i = 1:length(pulseMeth)
        ...
        pFRStimStruct{i} = PeakFRAligned(filteredSpikeArrayOnSet,trialNoStim{i},neuronNo,numSamples);
        
        disp('calculate peak firing rate for stimulated ctrl trials');
        pFRStimCtrlStruct{i} = PeakFRAligned(filteredSpikeArrayOnSet,trialNoStimCtrl{i},neuronNo,numSamples);
    end
