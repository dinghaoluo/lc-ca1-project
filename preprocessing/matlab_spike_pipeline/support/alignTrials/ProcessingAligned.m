...
% 0.5s start cue active --- condition 4 
    ...
    alignToRunOnset1(path, fileName,mazeSess);
   
    disp('Align trials based on reward onset')
    alignToReward1(path, fileName,mazeSess); % change by Yingxue and Dinghao on 3/4/2022, add the spikes before reward
...
%     parpool('local',4);
    ...
    meanNeuronSpikeCorrTStim(path,fileName,onlyRun,mazeSess,corrIntervalT);
    ...
    meanNeuronSpikeCorrDistStim(path,fileName,onlyRun,mazeSess,corrIntervalD);
    ...
    meanSpikeTrainSimilarityTStim(path,fileName,onlyRun,mazeSess,corrIntervalT);
    ...
    meanSpikeTrainSimilarityVPStim(path,fileName,onlyRun,mazeSess,cost,corrIntervalT);
    ...
    meanSpikeTrainSimilarityVPIStim(path,fileName,onlyRun,mazeSess,cost,corrIntervalT);
    ...
    meanSpikeTrainSimilarityVanRossumStim(path,fileName,onlyRun,mazeSess,tc,corrIntervalT);
    ...
    meanPopSpikeCorrTStim(path,fileName,onlyRun,mazeSess,corrIntervalT);
    ...
    meanPopSpikeSimTStim(path,fileName,onlyRun,mazeSess,corrIntervalT);
% 
%     % calculate spatial information
    disp('Calculate spatial information');
    GetFRMapInfo_Aligned(path,fileName,mazeSess,onlyRun,intervalTSpInfo);

    disp('Calculate spatial information (aligned to cue)');
    GetFRMapInfo_AlignedCue(path,fileName,mazeSess,onlyRun,intervalTSpInfo);
    ...
    FieldDetectionAligned(path,fileName,onlyRun,0,mazeSess,intervalTSpInfo); % mazeSess is only used for plotting purpose

    meanNeuronSpikeCorrTStim(path,fileName,onlyRun,mazeSess,intervalTSpInfo);
    FieldDetectionAlignedStim(path,fileName,onlyRun,0,mazeSess,intervalTSpInfo);
