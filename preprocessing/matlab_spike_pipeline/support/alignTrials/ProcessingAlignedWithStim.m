function ProcessingAlignedWithStim(path,fileName,onlyRun,mazeSess,cond,frProfile)
...
% 
% example:
% ProcessingAlignedWithStim('./','A020r-20220308-03_DataStructure_mazeSection1_TrialType1',0,1,0,3)
% 
% Dinghao 11 March 2022
% frProfile:
% 0: none
% 1: only aligned to run
% 2: only aligned to reward
% 3: both
    ...
    alignToReward1(path, fileName,mazeSess);
...
%     RunSpeedOverDistAligned(path, fileName, onlyRun, mazeSess);
    ...
    %% cue  aligned
%     disp(['Convolve spike trains with Gaussian kernel. only for spikes aligned' ...
%         'to cue, included spikes before cue']);
%     ConvSpikeTrain_AlignedOnset(path,fileName,onlyRun,mazeSess,3);
    ...
    if(cond == 2)
        ...
        plotSpikeRaster_GoodVsBadBeh(path, fileName, onlyRun, mazeSess, cond, frProfile);  
