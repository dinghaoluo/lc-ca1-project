function RunSpeedOverDistAligned(path, fileName, onlyRun, mazeSess)
    ...
    for i = 1:length(pulseMeth)
        speedOverDist.meanRunStim{i} = mean(speedOverDist.Run(trialNoStim{i},:));
        speedOverDist.stdRunStim{i} = std(speedOverDist.Run(trialNoStim{i},:));
        speedOverDist.SEMRunStim{i} = std(speedOverDist.Run(trialNoStim{i},:))...
            /sqrt(length(trialNoStim{i}));
    
        %% added by Yingxue on 2/14/2021
        speedOverDist.meanRunStimCtrl{i} = mean(speedOverDist.Run(trialNoStimCtrl{i},:));
        speedOverDist.stdRunStimCtrl{i} = std(speedOverDist.Run(trialNoStimCtrl{i},:));
        speedOverDist.SEMRunStimCtrl{i} = std(speedOverDist.Run(trialNoStimCtrl{i},:))...
