...
for i = 1:numTrials
    if(isempty(thetaCycleStruct.indCycleStart{indLapList(i)})) % there was a bug here, should be indLapList(i) instead of i. corrected on 2/19/2021 
        continue;
    end
    thetaIndLfp = thetaCycleStruct.indCycleStart{indLapList(i)} - nSampBef; % there was a bug here, should be indLapList(i) instead of i. corrected on 2/19/2021 
    ...
    spikeThetaPhaseStruct.indCycleStartAroundZero(i) = ...
        thetaCycleStruct.indCycleStart{indLapList(i)}(ind) - nSampBef;
    for j = 1:numNeurons
        ...
        % (***** cycle is not quite right when backward == 1 *********)
            for k = 1:thetaCycleStruct.numCyclesPerTrial(indLapList(i))
                indSpikesInCycle = find(spTimePerTrialPerNeuron >= ...
                    thetaCycleStruct.indCycleStart{indLapList(i)}(k) - nSampBef & ...
                    spTimePerTrialPerNeuron < ...
                    thetaCycleStruct.indCycleStart{indLapList(i)}(k) - nSampBef ...
                    +thetaCycleStruct.cycleLenPerTrial{indLapList(i)}(k));
