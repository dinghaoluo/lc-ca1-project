 function ThetaPhaseP2PRunOnset(path,fileName,method,onlyRun,mazeSess)
...
% ThetaPhaseP2PRunOnset('./','A111-20150301-01_DataStructure_mazeSection1_TrialType1',1,1,1)
    ...
    for i = trialNo
        if(method == 0) % hilbert
            thetaPh{i} = [trialsRun.thetaPhHilbBef{i}',trialsRun.thetaPhHilb{i}'];
            
        else % linear
            thetaPh{i} = [trialsRun.thetaPhLinInterpBef{i}',trialsRun.thetaPhLinInterp{i}'];
        end
    end
    ...
    spikeThetaPhaseRunNoStimGood = SpikeThetaPhaseRunOnset(trialsRunSpikes.TimeBef,...
        trialsRunSpikes.Time, trialsRunSpikes.thPhaseInterpSpikeBef,...
        trialsRunSpikes.thPhaseInterpSpike, thetaCycleRun, trialNoNonStimGood, noNeurons);
    
    disp('Calculate spike theta phase for non-stimulated bad trials')
    spikeThetaPhaseRunNoStimBad = SpikeThetaPhaseRunOnset(trialsRunSpikes.TimeBef,...
        trialsRunSpikes.Time, trialsRunSpikes.thPhaseInterpSpikeBef,...
        trialsRunSpikes.thPhaseInterpSpike, thetaCycleRun, trialNoNonStimBad, noNeurons);
    ...
    for i = 1:length(pulseMeth)
        ...
        spikeThetaPhaseRunStim{i} = SpikeThetaPhaseRunOnset(trialsRunSpikes.TimeBef,...
            trialsRunSpikes.Time, trialsRunSpikes.thPhaseInterpSpikeBef,...
            trialsRunSpikes.thPhaseInterpSpike, thetaCycleRun, trialNoStim{i}, noNeurons);
    
        disp('Calculate spike theta phase for control trials during stimulation')
        spikeThetaPhaseRunStimCtrl{i} = SpikeThetaPhaseRunOnset(trialsRunSpikes.TimeBef,...
            trialsRunSpikes.Time, trialsRunSpikes.thPhaseInterpSpikeBef,...
            trialsRunSpikes.thPhaseInterpSpike, thetaCycleRun, trialNoStimCtrl{i}, noNeurons);
    end
