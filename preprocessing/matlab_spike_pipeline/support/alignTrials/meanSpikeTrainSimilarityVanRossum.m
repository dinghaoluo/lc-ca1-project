...
% e.g.: meanSpikeTrainSimilarityVanRossum('./','A011-20190218-01_DataStructure_mazeSection1_TrialType1',1,100,8)
    ...
    for n = 1:neuronNo
        ...
        noStimGoodTrSim = triu(spikeTrainSimVRRun{n}(trialNoNonStimGood,trialNoNonStimGood),1);
        ...
        meanSimVRRun.meanNoStimGood(n) = sum(noStimGoodTrSim(isnan(noStimGoodTrSim) == 0))/nElemNoStimGood;
        ...
        meanSimVRRun.meanNoStimGoodNZ(n) = sum(noStimGoodTrSim(isnan(noStimGoodTrSim) == 0))/nElemNoStimGoodNonZeroTrRun;
        meanSimVRRun.nNoStimGoodNonZeroTr(n) = nNoStimGoodNonZeroTrRun;
        
        noStimBadTrSim = triu(spikeTrainSimVRRun{n}(trialNoNonStimBad,trialNoNonStimBad),1);
        noStimBadTrSim = noStimBadTrSim(:);
        nNoStimBad = length(trialNoNonStimBad);
        nElemNoStimBad = (nNoStimBad*nNoStimBad-nNoStimBad)/2;
        meanSimVRRun.meanNoStimBad(n) = sum(noStimBadTrSim(isnan(noStimBadTrSim) == 0))/nElemNoStimBad;
        ...
        meanSimVRRun.meanNoStimBadNZ(n) = sum(noStimBadTrSim(isnan(noStimBadTrSim) == 0))/nElemNoStimBadNonZeroTrRun;
        meanSimVRRun.nNoStimBadNonZeroTr(n) = nNoStimBadNonZeroTrRun;
        
        meanSimVRRun.meanStim = [];
        meanSimVRRun.meanStimNZ = [];
        meanSimVRRun.nStimNonZeroTr = [];
        meanSimVRRun.meanStimCtrl = [];
        meanSimVRRun.meanStimCtrlNZ = [];
        meanSimVRRun.nStimCtrlNonZeroTr = [];
        for i = 1:length(pulseMeth) 
            stimTrSim = triu(spikeTrainSimVRRun{n}(trialNoStim{i},trialNoStim{i}),1);
            stimTrSim = stimTrSim(:);
            nStim = length(trialNoStim{i});
            nElemStim = (nStim*nStim-nStim)/2;
            meanSimVRRun.meanStim{i}(n) = sum(stimTrSim(isnan(stimTrSim) == 0))/nElemStim;
            ...
            meanSimVRRun.meanStimNZ{i}(n) = sum(stimTrSim(isnan(stimTrSim) == 0))/nElemStimNonZeroTrRun;
            meanSimVRRun.nStimNonZeroTr{i}(n) = nStimNonZeroTrRun;

            stimCtrlTrSim = triu(spikeTrainSimVRRun{n}(trialNoStimCtrl{i},trialNoStimCtrl{i}),1);
            stimCtrlTrSim = stimCtrlTrSim(:);
            nStimCtrl = length(trialNoStimCtrl{i});
            nElemStimCtrl = (nStimCtrl*nStimCtrl-nStimCtrl)/2;
            meanSimVRRun.meanStimCtrl{i}(n) = sum(stimCtrlTrSim(isnan(stimCtrlTrSim) == 0))/nElemStimCtrl;
            ...
            meanSimVRRun.meanStimCtrlNZ{i}(n) = sum(stimCtrlTrSim(isnan(stimCtrlTrSim) == 0))/nElemStimCtrlNonZeroTrRun;
            meanSimVRRun.nStimCtrlNonZeroTr{i}(n) = nStimCtrlNonZeroTrRun;
        end
        %%
    end
