...
% plot spike rasters and separate trials based on the animal behavior
    ...
    disp(['Number of good trials = ' num2str(length(trialNoNonStimGood))]);
    
    plotSpikePhase_aligned(path, fileName, onlyRun, mazeSess, methodTheta, trialNoNonStimGood, trialNoNonStimBad);
end
...
%             figTitle = 'Spikes vs Dist';
        ...
        subplot(6,2,mod(count-1,12)+1)
        hold on;
        if(~isempty(spikeThetaPhaseRunNoStimBad))
