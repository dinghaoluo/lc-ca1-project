function plotSpikeRaster_aligned(path, fileName, onlyRun, mazeSess, trialNo1, cond, neuronNo)
...
%             disp(['Trial ' num2str(j)])
            ...
            plot([trialsRunSpikes.TimeBef{i,trialNo1(j)}'/sampleFq ...
                 ...
                 trialsRunSpikes.Time{i,trialNo1(j)}'])),'k.',...
                 'MarkerSize',3);    
...
%                  'MarkerSize',3);     
        ...
        for j = 1:(length(trialNo1)-1)   
            ...
            if(cond == 1)
                ...
                plot([trialsRewSpikes.TimeBef{i,trialNo1(j)}'/sampleFq ...
                     ...
                     trialsRewSpikes.Time{i,trialNo1(j)}'])),'k.',...
                     'MarkerSize',3);    
            end
        end
        if(~isempty(indTrialCut))
        ...
        % plot firing rate curve for clearer visualisation
%         h = plot(timeStepRun/sampleFq,pFRNonStimGoodStruct.avgFRProfile(i,:),'r-');
%         set(h,'LineWidth',0.5);
        ...
        if(cond == 1)
            ...
            set(gca, 'XLim', [-1*nSampBefRew/sampleFq trialLenT]);
        end
        %%
        if(mod(count-1,3) == 0)
            ylabel('Trial no.');
