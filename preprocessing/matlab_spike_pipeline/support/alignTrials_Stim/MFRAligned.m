function [mFRStruct] = MFRAligned(spikes,indLapList,numNeurons,lenTrialsT,indRunInLap)
...
for j = 1:numNeurons
    mFRStruct.spPerNeuron{j} = []; 
    for i = 1:numTrials
%         disp(['Neu ' num2str(j) ' Tr ' num2str(i)])
        spPerLap = spikes{j,indLapList(i)};
        ...
        numSpikes = length(spPerLap);
        if(numSpikes ~= 0)
            mFRStruct.spPerNeuron{j} = [mFRStruct.spPerNeuron{j}; spPerLap];
            mFRStruct.mFRArr(j,i) = numSpikes/lenTrial;
        end
    end
end

mFRStruct.mFR = mean(mFRStruct.mFRArr,2)';
mFRStruct.stdMFR = std(mFRStruct.mFRArr,0,2)';