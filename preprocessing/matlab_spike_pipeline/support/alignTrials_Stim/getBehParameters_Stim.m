function getBehParameters_Stim(path,fileName,mazeSess)
...
% e.g. getBehParameters('./','A011-20190218-01_DataStructure_mazeSection1_TrialType1',1)
    ...
    %% changed  on 2/3/2021 by Yingxue
    behPar.indLapsSess = find(Laps.mazeSess == mazeSess);
    if(isfield(beh,'indStimLap'))
        behPar.indLapsStimSess = find(Laps.mazeSess == mazeSess & beh.indStimLap == isStim);
        [~,behPar.indLapsStimInSess] = intersect(behPar.indLapsSess,behPar.indLapsStimSess);
        [behPar.indLapsNonStimSess,behPar.indLapsNonStimInSess] = setdiff(behPar.indLapsSess,behPar.indLapsStimSess);
        
        goodTr = find(beh.indStimLap(behPar.indLapsSess) == 0 & behPar.indTrBadBeh == 0);
        beh.indLapsNonStimGood 
        ...
        behPar.indLapsStimSess = [];
        behPar.indLapsStimInSess = [];
        behPar.indLapsNonStimSess = behPar.indLapsSess;
        behPar.indLapsNonStimInSess = 1:length(behPar.indLapsNonStimSess);
    end
    ...
    for i = trialNo
        if(~isempty(trialsRun.xMM{i}))
            ...
            behPar.rewarded(i) = trialsRew.goodTrial(i);
