function plotSpikePhaseCueOffset(path, fileName, onlyRun, mazeSess)
...
% E.G.: plotSpikeRaster_aligned('./','A002-20181005-01_DataStructure_mazeSection1_TrialType',1)
    ...
    fullPath = [path fileName '_PeakFR_msess' num2str(mazeSess) '_CueOffRun' num2str(onlyRun) '.mat'];
    if(exist(fullPath) == 0)
    ...
    fullPath = [path fileName '_convSpikesAligned_msess' num2str(mazeSess) '_BefCueOffRun' num2str(onlyRun) '.mat'];
...
%             figTitle = 'Spikes vs Dist';
        ...
        for j = trialNoG   
            ...
            plot([trialsCueOffSpikes.TimeBef{i,j}'/sampleFq ...
                 ...
                 [trialsCueOffSpikes.thPhaseInterpSpikeBef{i,j}' ...
                 trialsCueOffSpikes.thPhaseInterpSpike{i,j}'],'k.',...
                 'MarkerSize',3);     
        end
        if(mod(count-1,2) == 0)
            ylabel('Theta phase');
