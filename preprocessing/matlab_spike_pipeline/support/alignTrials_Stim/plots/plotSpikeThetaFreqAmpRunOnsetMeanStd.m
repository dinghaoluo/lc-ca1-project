...
% E.G.: plotSpikeRaster_aligned('./','A002-20181005-01_DataStructure_mazeSection1_TrialType',1)
    ...
    if(isfield(beh,'indStimLap'))
        ...
        trialNoG = find(beh.indStimLap(ind) == 0 & behPar.indTrBadBeh == 0);
        trialNoB = find(beh.indStimLap(ind) == 0 & behPar.indTrBadBeh == 1);
        trialNoStim = find(beh.indStimLap(ind) == 1);
    else
        trialNoG = find(behPar.indTrBadBeh == 0);
        trialNoB = find(behPar.indTrBadBeh == 1);
        trialNoStim = [];
    end
    ...
    trialLenT = 7; %sec
    meanThetaNonStimGood = calTheta(trialsRun,trialNoG,trialLenT,nSampBef,sampleFq);
    meanThetaNonStimBad = calTheta(trialsRun,trialNoB,trialLenT,nSampBef,sampleFq);
    meanThetaStim = calTheta(trialsRun,trialNoStim,trialLenT,nSampBef,sampleFq);
    t = (-nSampBef+1:trialLenT*sampleFq)/sampleFq;
    
    fullPath = [path fileName '_ThetaAlignRun_msess' num2str(mazeSess) '.mat']; 
    ...
    fileName1 = ['Z:\Yingxue\DataAnalysisRaphi\' ...
        fileName(1:ind(1)) 'ThetaFreqVsTAlignedToRun'];
    ...
    fileName1 = ['Z:\Yingxue\DataAnalysisRaphi\' ...
        fileName(1:ind(1)) 'ThetaAmpVsTAlignedToRun'];
    saveas(gcf,fileName1);
    print('-painters', '-dpdf', fileName1, '-r600')
    ...
    fileName1 = ['Z:\Yingxue\DataAnalysisRaphi\' ...
        fileName(1:ind(1)) 'SpeedVsTAlignedToRun'];
    saveas(gcf,fileName1);
    print('-painters', '-dpdf', fileName1, '-r600')
