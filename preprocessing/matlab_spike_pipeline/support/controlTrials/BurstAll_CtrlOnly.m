function BurstAll_CtrlOnly(path,fileName,methodTheta,onlyRun,mazeSess)
...
%
% Example:
% BurstAll_CtrlOnly('./','xzvr_PR1-20170727-01_DataStructure_mazeSection1_TrialType1',1,0,1)
    ...
    fileNameBurst = [fileName '_burstAll_Ctrl_TH' th '_Run' num2str(onlyRun) ...
    ...
    if(~isempty(mFRStructSessCtrl))
        indLaps = mFRStructSessCtrl.indLapList; 
        burstPerNeuPerTrSessCtrl = ...
            BurstPerNeuronPerTrial(spikes,theta,dist,...
            indLaps,paramBurst);
      
        disp('Calculate bursts per neuron')
        burstIsiPerNeuronSessCtrl = BurstPerNeuron(burstPerNeuPerTrSessCtrl,...
                               indLaps,rec.numNeurons);
    end
       
    %%%%%%%%% save data
    fullPath = [path fileNameBurst];
    save(fullPath, 'burstPerNeuPerTrSessCtrl','burstIsiPerNeuronSessCtrl',...
         'paramBurst');
