function PeakFiringRate_AlignedRunOnsetCtrlOnly(path,fileName,fileState,onlyRun,mazeSess,figureState)
...
% PeakFiringRateVR('./','A111-20150301-01_DataStructure_mazeSection1_TrialType1',1,1,1,1)
    ...
    fileNamePeakFR = [fileName '_PeakFRCtrl_msess' num2str(mazeSess) '_RunOnset' num2str(onlyRun) '.mat'];        
    if(fileState == 0)
    ...
    disp('calculate peak firing rate for non-stimulated trials');
    pFRNonStimStruct = PeakFRAligned(filteredSpikeArrayRunOnSet,trialNoNonStim,neuronNo,numSamples);
       
    save([path fileNamePeakFR], 'pFRNonStimStruct','-v7.3');
