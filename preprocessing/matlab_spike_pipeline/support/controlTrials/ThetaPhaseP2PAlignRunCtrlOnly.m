 function ThetaPhaseP2PAlignRunCtrlOnly(path,fileName,method,onlyRun,mazeSess)
...
% ThetaPhaseP2PRunOnset('./','A111-20150301-01_DataStructure_mazeSection1_TrialType1',1,1,1)
    ...
    if(method == 0)
        ...
        fileNameThetaPhaseCtrl = [fileName '_ThetaPhaseHAlignedCtrl_msess' num2str(mazeSess) '_Run' num2str(onlyRun) '.mat'];
    else
        fileNameThetaPhase = [fileName '_ThetaPhaseLAligned_msess' num2str(mazeSess) '_Run' num2str(onlyRun) '.mat'];
        fileNameThetaPhaseCtrl = [fileName '_ThetaPhaseLAlignedCtrl_msess' num2str(mazeSess) '_Run' num2str(onlyRun) '.mat'];
    end
    fullPath = [path fileNameThetaPhase];
    if(exist(fullPath) == 0)
        disp('The _ThetaPhaseLAligned file does not exist');
        return;
    end
    ...
    spikeThetaPhaseRunNoStim = SpikeThetaPhaseAlignedRun(trialsRunSpikes.Time,...
        thPhase, thetaCycleRun, trialNoNonStim, noNeurons);
