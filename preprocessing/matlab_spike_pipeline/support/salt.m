...
% Code from the Kepecs Laboratory from following URL:
% http://kepecslab.cshl.edu/salt.m
% method used in following paper:
% https://www.cell.com/fulltext/S0092-8674(15)00973-3
% https://github.com/buzsakilab/buzcode/tree/master/analysis/spikes
% https://www.cell.com/cms/10.1016/j.cell.2015.07.057/attachment/5e2bb27e-a8b1-4c34-bc8b-b6f539ca015d/mmc1.pdf
...
for t = 1:nmbn:st   % loop through baseline windows
    ...
    % added this part so the middle distribution does not have identical support
    % add epsilon to every entry, then renormalise
    nhlsi(:,next) = nhlsi(:,next) + 0.0001;
    nhlsi(:,next) = nhlsi(:,next) / sum(nhlsi(:,next));
...
% same adjustment for the test histogram
nhlsi(:,next) = nhlsi(:,next) + 0.0001;
nhlsi(:,next) = nhlsi(:,next) / sum(nhlsi(:,next));

% % added this  plot for debugging
% for i = 1:next
%     figure(i);  
%     histogram(hlsi(:,i), edges);    
% end
% 
