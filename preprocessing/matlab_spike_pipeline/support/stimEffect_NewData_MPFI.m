...
% edited Dinghao 5 Sept 2022
    ...
    % check arguments
    if nargin<2
        disp('At least two arguments are needed for stimEffect().');
        return;
    end
    ...
    if(isempty(indexFileName))
        fileNameFR = [fileName '_FR_Run0.mat'];
        fileNameStim = [fileName '_stimEff.mat'];
        fileName = [fileName '.mat'];
    else
        fileNameFR = [fileName(1:indexFileName(end)-1) '_FR_Run0.mat'];
...
%     numNeurons = length(stims{1}.spikes);
    numNeurons = length(cluList.all); % alternatively coded before finding out where stims is created, Dinghao 22 Jun 2022
    numStimDiodes = length(stims{1}.indDiode);
    ...
    stimResp = struct('indNeurons',1:numNeurons,...
                     ...
                     'stimShank',zeros(numStim,numStimDiodes),...
    ...
    for j = 1:numNeurons
        ...
        for i = 1:numStim
            ...
            numPulsesInStim  = max(stims{i}.indPulseInStim);
            ...
            for m = 1:numPulsesInStim
                ...
                indBef = stims{i}.spikesPulseNoBef{m,j} == m; % spikes before mth pulse for the jth neuron
                ...
                indAft = stims{i}.spikesPulseNoAft{m,j} == m;
            ...
            spikesAft = [stims{i}.spikes{j}; repmat(stims{i}.spikesAft{j},numStim,1)];  % WHY ARE WE DOING THIS??
    ...
    for i = 1:numStim
        ...
        for j = 1:numNeurons
            ...
            numTrainsInStim = 1; %max(stims{i}.indTrainInStim);
            ...
            for m = 1:numPulsesInStim
                ...
                indBef = stims{i}.spikesPulseNoBef{m,j} == m; %stims{i}.SpikesPulseNoBef{indTrain,j} == stims{i}.indPulseInStim(indTrain1st);               
                stimResp.spPerPulseBef{i,j}{m} = stims{i}.spikesBef{m,j}(indBef);
                
                indAft = stims{i}.spikesPulseNoAft{m,j} == m; %stims{i}.SpikesPulseNoAft{indTrain,j} == stims{i}.indPulseInStim(indTrainlast);
                indStim = stims{i}.spikesPulseNo{m,j} == m; %stims{i}.SpikesPulseNo{indTrain,j} == stims{i}.indPulseInStim(m);
                stimResp.spPerPulseAft{i,j}{m} = [stims{i}.spikes{m,j}(indStim); stims{i}.spikesAft{m,j}(indAft)];
...
%             end
            ...
            if(ismember(cluList.shank(j), stimResp.stimShank(i)))  %(cluList.shank(j) == stimResp.stimShank(i))
...
%                 pause;
    ...
    % Run Salt Analysis with baseline parameters
    base_time = 500; % ms
    time_res = 1; 
    window_size = 10;
    runSaltAll('./', fileNameCopy, base_time, time_res, window_size);
