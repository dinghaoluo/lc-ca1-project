function ts=videoframets(varargin)
    ...
    %       The same pipe-to-file commands are expected to work on OSX/UNIX,
    %       but this copy was tested on Windows.
    %
    %   EXAMPLE:
    %       ffpath='C:\ffmpeg\bin'
    ...
    %   deviate from this value. This can happen when the encoder is delayed, for
    %   instance if the computer recording video is under excessive load and frames arrive
    %   more slowly. If frames often come in a little bit slower than expected (empirical 
...
%       from system. It is actually orders of
%       magnitude faster (?)
%
%       2: ffmpeg tests showed that ffmpeg's showinfo surprisingly seems to
%       put its output into the stderr output (i.e. use 2> rather than >).
