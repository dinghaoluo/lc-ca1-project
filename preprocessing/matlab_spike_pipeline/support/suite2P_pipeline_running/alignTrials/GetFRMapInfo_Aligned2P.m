function GetFRMapInfo_Aligned2P(path, fileName, mazeSess, onlyRun, intervalT)
...
% by Yingxue, 2017.08.24
    ...
    %% non stimulated good trials
    disp('Spatial information for non stimulated good trials')
    spatialInfoSessNonStimGood = spatialInfoSessNonStim;
    indLaps = setdiff(trialNoNonStimGood,1:startTrNo);
    spatialInfoSessNonStimGood = ...
    calSpatialInfo(indLaps,trialsRun,...
        spatialInfoSessNonStimGood,totClu,param,drawFig);

    %% non stimulated bad trials
    disp('Spatial information for non stimulated bad trials')
    spatialInfoSessNonStimBad = spatialInfoSessNonStim;
    indLaps = setdiff(trialNoNonStimBad,1:startTrNo);
    spatialInfoSessNonStimBad = ...
    calSpatialInfo(indLaps,trialsRun,...
        spatialInfoSessNonStimBad,totClu,param,drawFig);

    %% stimulated trials
    spatialInfoSessStim = [];
    spatialInfoSessStimCtrl = [];
    for i = 1:length(pulseMeth)
        disp('Spatial information for stimulated trials')
        spatialInfoSessStim{i} = spatialInfoSessNonStim;
        indLaps = setdiff(trialNoStim{i},1:startTrNo);
        spatialInfoSessStim{i} = ...
        calSpatialInfo(indLaps,trialsRun,...
            spatialInfoSessStim{i},totClu,param,drawFig);
    
        disp('Spatial information for stimulated ctrl trials')
        spatialInfoSessStimCtrl{i} = spatialInfoSessNonStim;
        indLaps = setdiff(trialNoStimCtrl{i},1:startTrNo);
        spatialInfoSessStimCtrl{i} = ...
        calSpatialInfo(indLaps,trialsRun,...
            spatialInfoSessStimCtrl{i},totClu,param,drawFig);
    end
    
    fullPath = [path fileNameSpInfo];
    save(fullPath, 'spatialInfoSessNonStimGood','spatialInfoSessNonStimBad',...
        'spatialInfoSessStim','spatialInfoSessStimCtrl');
end

function spatialInfoSessNonStim = ...
    calSpatialInfo(trialNoNonStim,trialsRun,...
    ...
    for j = 1:length(indLaps)
        ...
        for n = 1:totClu
            spikesTr{n} = [spikesTr{n}; trialsRun.spikes{indLaps(j)}(ind,n)];
        end
    end
    
    for j = 1:totClu
        if(sum(spikesTr{j}) > 0)
            spatialInfoSessNonStim.meanFR(j) = sum(spikesTr{j}) / ...
