function plotSequenceDist2P(path, fileName, onlyRun, mazeSess, intervalT, spaceBin, neuSel, neuArr, seqLabel)
% plot the sequence from one recording, align the sequence to different
% behavioral features
% examples: 
% 1. plot all the neurons with fields: plotSequenceDist2P('./','A576-20210914-02_DataStructure_mazeSection1_TrialType1',1,1,10,20,0,[],[])
% 2. plot a selected array of neurons: plotSequenceDist2P('./','A576-20210914-02_DataStructure_mazeSection1_TrialType1',1,1,10,20,2,[1 3 5],'PC1')
    ...
    fileName1 = [fileName(1:indstr(1)-1) '-SeqDist-GoodTr' filenameEnd];
    if(~isempty(seqLabel))
...
%     print('-painters','-dpdf',['Z:\Yingxue\DataAnalysisXiaoliang\2PData\' fileName1],'-r600');
    ...
    if(~isempty(badTrials))
        plotSequenceDist1(avgFilteredSpikeArrayRunBad,...
            spaceSteps{1},'Dist (cm)','Neuron No.','Firing rate bad trials');
        indstr = strfind(fileName,'_');
        fileName1 = [fileName(1:indstr(1)-1) '-SeqDist-BadTr' filenameEnd];
        if(~isempty(seqLabel))
            fileName1 = [fileName1 '-' seqLabel];
        end
        savefig(['Z:\Yingxue\DataAnalysisXiaoliang\2PData\' fileName1 '.fig'])
