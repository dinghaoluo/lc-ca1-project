...
%    GetTrials_JF_v4('A132-20111025-01',1,1,0,0);
    ...
    if exist([filename '_BehavElectrDataLFP.mat'],'file') == 2 ...
        ...
        load([filename '_BehavElectrDataLFP.mat'],...
            'Track','Laps','Spike','Clu','CCG','Stim');
    else
        GenerateBehavElectroDataStructures_MPFIv1(pwd, filename);
        load([filename '_BehavElectrDataLFP.mat'],...
                'Track','Laps','Clu','Spike','CCG','Stim'); 
    end
    ...
    if(stimOnlyN == 0)
        ...
        lfpIndStartAll = cell(1,nTotClu);
        lfpIndEndAll = cell(1,nTotClu);
        if mazeSection == 1 ...
                & ~isempty(strfind(epochGroup.protocol.purpose,'Treadmill'))
            %%%%%%% RUNNING WHEEL %%%%%%%%%%%
    ...
    for nStim = 1:totStim
        disp(['Stimulation ' num2str(nStim)]);
        indCurStim = find(Stim.indStim == nStim);
        if(isempty(indCurStim)) 
            % added 12.28.2015 to account for the stimulation error
            % happened before the pulse definition
            continue;
        end
        ...
        stims{nStim}.indStim = nStim;
        stims{nStim}.indDiode = unique(Stim.indDiode(indCurStim));
        stims{nStim}.indPulseInStim = Stim.indPulseInStim(indCurStim);
        stims{nStim}.indTrainInStim = Stim.indTrainInStim(indCurStim);
        stims{nStim}.pulsePeriod = unique(Stim.pulsePeriod(indCurStim));
        numTrain = max(Stim.indTrainInStim(indCurStim));
        ...
        for nClu = 1:nTotClu
            if ~isempty(find(cluList1 == nClu, 1))
                ...
                for nCurStim = 1:nCurStimLen
                    ...
                    indTrain = Stim.indTrainInStim(indStimTemp);
                    % spikes during the stimulation
                    allSpInd = res(res >= Stim.startLfpInd(indStimTemp) ...
                    ...
                    allSpInd = res(res > Stim.stopLfpInd(indStimTemp) ...
