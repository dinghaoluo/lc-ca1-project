%function [xml, rxml] = LoadXml_e(FileBase)
...
function [xml, rxml] = LoadXml_e(FileBase,varargin)
xml = struct;
...
for i=1:length(rxml.child)
    ...
    switch rxml.child(i).tag
        ...
        case 'acquisitionSystem'
            for nChild = 1 : length(rxml.child(i).child)
                switch nChild
                    case 1; xml.nBits = str2num(rxml.child(i).child(1).value); % number of bits of the file
                    case 2; xml.nChannels = str2num(rxml.child(i).child(2).value);
                    case 3; xml.SampleRate = str2num(rxml.child(i).child(3).value);
                            xml.SampleTime = 1e6/xml.SampleRate; %to make backwards compatible
                    case 4; xml.VoltageRange = str2num(rxml.child(i).child(4).value);
                    case 5; xml.Amplification = str2num(rxml.child(i).child(5).value);
                    case 6; xml.Offset =  str2num(rxml.child(i).child(6).value);
                end
            end
        ...
        case 'channelGroups'
            ...
            for grpI =1:length(tmp)
                for chI=1:length(tmp(grpI).child)
                    xml.AnatGrps(grpI).Channels(chI) = str2num(tmp(grpI).child(chI).value);        
                    if length(str2num(tmp(grpI).child(chI).attribs.value)) > 0
                         xml.AnatGrps(grpI).Skip(chI) = str2num(tmp(grpI).child(chI).attribs.value);
                    else xml.AnatGrps(grpI).Skip = [];
                    end
                end
            end
