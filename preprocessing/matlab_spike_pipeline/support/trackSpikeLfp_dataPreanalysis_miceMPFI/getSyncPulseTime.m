function [UpCrossings] = getSyncPulseTime(baseFileName,nChannelsTot,sampleRate)
 ...
 % 26 - file size (bytes)
    ...
    [up down] = TriggerUpDownMarked(lfSync,(mean(lfSync)+amp/6),(mean(lfSync)+amp)/3);
    ...
    if(~isempty(down) && ~isempty(up))
        ...
        for i = 1:lenTmp
            ...
            tmpMatch = find(tmpDiff > sampleRate*0.0085 & tmpDiff < sampleRate*0.0115);
