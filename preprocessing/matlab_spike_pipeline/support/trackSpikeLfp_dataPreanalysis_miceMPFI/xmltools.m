...
if ~isstr(arg)
  ...
  if length(varargin) >= 1 %& ~FileExists(arg)
    ...
    switch upper( cmode)
     ...
     case 'GET'
      %< Get the subset
      % child selection uses the child field below
      next = 'child';
...
while ~eot & ~isempty(udeblank(deblank(str)))
  ...
  %< No more tags remain in the document
  ...
  if f_end <= f_beg
    ...
    z.tag     = current_tag;        % originally, this fce was using upper(current_tag) but Neruoscope would not read that
