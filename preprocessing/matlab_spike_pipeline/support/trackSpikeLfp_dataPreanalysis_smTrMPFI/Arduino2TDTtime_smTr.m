...
% using the sync signal to align the arduino time and the TDT time
   ...
   %%%%%%%% convert the arduino time stamps to the TDT time stamps
    ...
    for i = 1:length(behEventsTdt.trialDescr(:,1))
        if(behEventsTdt.movieTDescr{i}(end) == 1) % one reward
            if(behEventsTdt.taskDescr(i,4) == 0)
                behEventsTdt.mazeType(i) = 1; % no cue
            elseif(behEventsTdt.taskDescr(i,4) == 3 ...
                    && length(behEventsTdt.movieTDescr{i}) == 2)
                behEventsTdt.mazeType(i) = 2; % start cue
            elseif(behEventsTdt.taskDescr(i,4) == 3 ...
                    && length(behEventsTdt.movieTDescr{i}) == 4)
                behEventsTdt.mazeType(i) = 3; % three cues
            end
        elseif(behEventsTdt.movieTDescr{i}(end) == 2) % double reward
            if(behEventsTdt.taskDescr(i,4) == 0)
                behEventsTdt.mazeType(i) = 4; % no cue
            elseif(behEventsTdt.taskDescr(i,4) == 3 ...
                    && length(behEventsTdt.movieTDescr{i}) == 2)
                behEventsTdt.mazeType(i) = 5; % start cue
            elseif(behEventsTdt.taskDescr(i,4) == 3 ...
                    && length(behEventsTdt.movieTDescr{i}) == 4)
                behEventsTdt.mazeType(i) = 6; % three cues
            end
        end
        if(i > 1 && ...
                (behEventsTdt.movieTDescr{i}(end) ~= ...
                behEventsTdt.movieTDescr{i-1}(end) || ...
                behEventsTdt.taskDescr(i,4) ~= ...
                behEventsTdt.taskDescr(i-1,4) || ...
                length(behEventsTdt.movieTDescr{i}) ~= ...
                length(behEventsTdt.movieTDescr{i-1})))
            mazeSess = mazeSess + 1;
        end
