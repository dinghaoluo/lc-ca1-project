...
% using the sync signal to align the arduino time and the TDT time
   ...
   %%%%%%%% convert the arduino time stamps to the TDT time stamps
    behEventsTdt = [];
    % added by Yingxue on 01/30/2019, to address the problem of a short
    % trial at the beginning of the recording
    if(behEvents.trialDescr(1,1) - behEvents.taskDescr(1,1) < 1000)
        behEvents.trialDescr = behEvents.trialDescr(2:end,:);
        behEvents.trialDescr(:,2) = behEvents.trialDescr(:,2) - 1;
        behEvents.taskDescr = behEvents.taskDescr(2:end,:);
        behEvents.movieTDescr = behEvents.movieTDescr(2:end);
    end
    ...
    indTrStartCues1 = ...
        find(behEvents.taskDescr(:,3) == 3);
    if(~isempty(indTrStartCues1))
        indTrStartCues = [];
        for i = 1:length(indTrStartCues1)
            if(length(behEvents.movieTDescr{indTrStartCues1(i)}) == 3 ...
                    && sum(behEvents.movieTDescr{indTrStartCues1(i)} == [0 60 120]) == 3)
                indTrStartCues = [indTrStartCues indTrStartCues1(i)];
            end
        end
        if(~isempty(indTrStartCues))
            if(indTrStartCues(1) > 1)
                behEventsTdt.mazeType(1:indTrStartCues(1)-1) = 7; % random cues
                behEventsTdt.mazeSess(1:indTrStartCues(1)-1) = mazeSess;
                mazeSess = mazeSess + 1;
            end
            behEventsTdt.mazeType(indTrStartCues(1:end-1))= 2; % start cue
            behEventsTdt.mazeSess(indTrStartCues(1:end-1)) = mazeSess;
            if(indTrStartCues(end) < length(behEventsTdt.trialDescr(:,1)))
                behEventsTdt.mazeType(indTrStartCues(end):end)= 7; % start cue
                behEventsTdt.mazeSess(indTrStartCues(end):end) = mazeSess+1;
            end
        else
            behEventsTdt.mazeType(:) = 7; % random cues
            behEventsTdt.mazeSess(:) = 1;
        end
    else
        behEventsTdt.mazeType(:) = 7; % random cues
        behEventsTdt.mazeSess(:) = 1;
