...
% this code is specifically modified to find sessions in ANM009 recordings 
% in Raphi's data 
% Modified by Yingxue Wang on 2019/01/23
   ...
   %%%%%%%% convert the arduino time stamps to the TDT time stamps
    ...
    indTrThreeCues = [];
    for tr = indTrThreeCuesTmp
        if(behEventsTdt.movieTDescr{tr}(end) == 1)
            indTrThreeCues = [indTrThreeCues tr];
        else
            break;
        end
    end
    ...
    % there was a mistake in the Arduino behavior code for ANM009 and
    % A007-20190116. This code is specifically targeting those three
    % recordings. The second no cue session has random Movie signals in the
    % behavior files
    indTrFreeRun = [];
    indTrDoubleReward = [];
    for tr = 1:length(behEventsTdt.mazeType)
        if(behEventsTdt.movieTDescr{tr}(end) == 0)
            indTrFreeRun = [indTrFreeRun tr];
        elseif(behEventsTdt.movieTDescr{tr}(end) == 2)
            indTrDoubleReward = [indTrDoubleReward tr];
        end
    end
