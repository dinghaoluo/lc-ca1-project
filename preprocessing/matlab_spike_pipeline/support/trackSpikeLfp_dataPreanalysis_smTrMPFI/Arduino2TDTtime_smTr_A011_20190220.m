function Arduino2TDTtime_smTr_A011_20190220(baseFileName,sampleFreq,lfpFreq)
   ...
   %%%%%%%% convert the arduino time stamps to the TDT time stamps
    ...
    if(length(behEventsTdt.taskDescr(:,1)) == length(behEventsTdt.trialDescr(:,1)))
        if(sum(behEventsTdt.trialDescr(:,1)-behEventsTdt.taskDescr(:,1) <= 0) > 0)
            if(behEventsTdt.trialDescr(1,3) == 0)
    ...
    for i = 1:length(behEventsTdt.trialDescr(:,1))
        indMovie = find(behEventsTdt.movieOn(:,1) > behEventsTdt.taskDescr(i,1) ...
            & behEventsTdt.movieOn(:,1) < behEventsTdt.trialDescr(i,1));
        numMovies = length(indMovie);
        if(numMovies == 3)
             behEventsTdt.mazeType(i) = 5; 
             % start cue, with active licking to receive reward
        elseif(numMovies == 5)
            behEventsTdt.mazeType(i) = 6; 
             % start cue, with active licking to receive reward, with a
             % middle cue from 40-80 cm
        end
        if(i > 1 && ...
