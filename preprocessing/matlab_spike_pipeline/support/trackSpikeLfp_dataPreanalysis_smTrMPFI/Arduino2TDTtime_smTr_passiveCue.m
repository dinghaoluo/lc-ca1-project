...
if(isfield(behEvents,'taskDescr'))
    ...
    for i = 1:size(behEvents.taskDescr,1)
        ...
        behEventsTdt.taskDescr(i,5) = behEvents.movieTDescr{i}(3); % cue position
    end
...
for i = 1:length(behEventsTdt.trialDescr(:,1))
    ...
    elseif(behEventsTdt.taskDescr(i,4) == 23) % added on 3/11/2020
        % passive licking with cue at different locations (0.5s cue)
        behEventsTdt.mazeType(i) = 23;  
    end
    if(behEventsTdt.mazeType(i) == 0)
        ...
        %% added on 3/11/2020 (for passive cue at different locations)
        if(i > 1 && behEventsTdt.mazeSess(i-1) ~= 0 && ...
                behEventsTdt.mazeType(i) == 23 && ...
