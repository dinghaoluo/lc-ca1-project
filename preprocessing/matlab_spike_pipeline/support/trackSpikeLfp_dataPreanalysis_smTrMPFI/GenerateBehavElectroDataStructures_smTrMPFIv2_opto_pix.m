function [Spike Clu Track Laps xml] = GenerateBehavElectroDataStructures_smallTMPFIv2_opto_pix(path, FileNameBaseList, varargin)
...
%     load behavior file
    ...
    check = Arduino2RecordT_smTr_opto_pix(FileNameBase,SampleRate,lfpSampleRate,nChannels);
    if(check == 1)
        return;
    end
...
%     Stim2RecordT_mpfi(FileNameBase,SampleRate,lfpSampleRate,nChannels);
    ...
    fprintf('\nAlign tracking data with .dat file...\n')
    aligntsp2dat_smTr_pix(FileNameBase,SampleRate,lfpSampleRate,nChannels);
...
%     end
    ...
    fprintf('\nGet theta phase and track information for spikes');
    getSpikeInfo_pix(FileNameBase,lfpSampleRate,strExt);
    
    % get Stim information
    fprintf('\nGet tagging stim information');
    getStim_mpfi_smTr_pix(FileNameBase, lfpSampleRate);
