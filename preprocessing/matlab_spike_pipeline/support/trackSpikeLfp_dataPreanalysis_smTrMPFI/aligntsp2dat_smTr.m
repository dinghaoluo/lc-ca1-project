...
if exist([file '-whl.mat'], 'file') == 2
    ...
    while ischar(tline)
        if startsWith(tline,'TimeStamp of the end')
            tline=tline(59:end);
            EndTimestamp=sscanf(tline,'%ld',1);
        end
        if startsWith(tline,'TimeStamp of the start')
            tline=tline(61:end);
            StartTimestamp=sscanf(tline,'%ld',1);
        end
    ...
    if EndTimestamp<0 & StartTimestamp<0
        ...
        timeStep = double(EndTimestamp-StartTimestamp)/(DatLength-1); % in ms
        timeStepLfp = timeStep*sampleFreq/lfpFreq;
        interpTsp(:,1) = 0:timeStep:double(EndTimestamp-StartTimestamp);
        whlDataLfp(:,1) = 0:timeStepLfp:double(EndTimestamp-StartTimestamp);
    end
